...
async function assertPortFree(port) {
  ...
  if (listening) {
    throw new Error(
      `Refusing to restore while a service is listening on port ${port}. Stop iclaw first (make stop).`,
    );
  }
}
...
// A `.iclaw-restore-*` staging directory (holding the pre-restore
...
function cleanupOrphanedRestoreStagingDirs(parentDir, excludeStageRoot) {
  ...
  for (const entry of entries) {
    if (!entry.isDirectory() || !entry.name.startsWith('.iclaw-restore-'))
      continue;
...
// in active use right now" — both just look like a `.iclaw-restore-*`
...
function acquireRestoreLock(parentDir) {
  const lockPath = path.join(parentDir, '.iclaw-restore.lock');
  try {
...
async function restore(archiveArg, dataDirArg, port) {
  ...
  try {
    ...
    stageRoot = fs.mkdtempSync(path.join(parentDir, '.iclaw-restore-'));
    runTar(
      [
        '-xzf',
        archivePath,

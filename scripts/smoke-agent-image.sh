...
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-iclaw-image-smoke-${GITHUB_RUN_ID:-local}-${RANDOM}}"
...
if [ "$smoke_host_uid" -eq 0 ]; then
  smoke_identity_args=(
    --env iclaw_HOST_IDENTITY_MODE=host-root
  )
else
  smoke_identity_args=(
    --env iclaw_HOST_IDENTITY_MODE=direct
    --env "iclaw_HOST_UID=$smoke_host_uid"
    --env "iclaw_HOST_GID=$smoke_host_gid"
  )
fi

...
BACKUP_DIR ?= .
CONTAINER_IMAGE ?= URBANSyndrazZ/iclaw-agent:latest
...
logs: ## 实时查看日志（需配合手动后台运行：make start > /tmp/iclaw.log 2>&1 &）
	@tail -f /tmp/iclaw.log

stop: ## 停止监听指定端口的服务进程
	@lsof -ti:$(PORT) -sTCP:LISTEN 2>/dev/null | xargs kill 2>/dev/null && echo "✅ 已停止 iclaw (端口 $(PORT))" || echo "⚠️  端口 $(PORT) 未被占用，无需停止"
...
status: ## 查看服务运行状态
	...
	@echo ""
	@echo "=== 日志文件 ==="
	@if [ -f /tmp/iclaw.log ]; then \
	  echo "✅ /tmp/iclaw.log 存在 ($$(wc -l < /tmp/iclaw.log) 行)"; \
	  echo "   最近 3 行:"; \
	  tail -3 /tmp/iclaw.log | sed 's/^/   /'; \
	else \
	  echo "⚠️  /tmp/iclaw.log 不存在（未用后台模式启动）"; \
	fi
	@echo ""
	@echo "=== Docker 容器 ==="
	@docker ps --filter "name=iclaw" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null || echo "   Docker 未运行或无 iclaw 容器"
...
backup: ## 备份运行时数据到 iclaw-backup-{date}.tar.gz
	@set -eu; \
	DATE=$$(date +%Y%m%d-%H%M%S); \
	mkdir -p "$(BACKUP_DIR)"; \
	FILE="$(BACKUP_DIR)/iclaw-backup-$$DATE.tar.gz"; \
	if [ -e "$$FILE" ]; then FILE="$(BACKUP_DIR)/iclaw-backup-$$DATE-$$$$.tar.gz"; fi; \
	TMP_FILE="$$FILE.tmp-$$$$"; \
	TMP_ROOT=$$(mktemp -d "$${TMPDIR:-/tmp}/iclaw-backup.XXXXXX"); \
...
restore: ## 从 iclaw-backup-*.tar.gz 恢复数据（用法：make restore 或 make restore FILE=xxx.tar.gz）
	@set -eu; \
	if [ -n "$(FILE)" ]; then \
	  BACKUP="$(FILE)"; \
	elif [ $$(find "$(BACKUP_DIR)" -maxdepth 1 -type f -name 'iclaw-backup-*.tar.gz' 2>/dev/null | wc -l) -eq 1 ]; then \
	  BACKUP=$$(find "$(BACKUP_DIR)" -maxdepth 1 -type f -name 'iclaw-backup-*.tar.gz' | head -1); \
	elif [ $$(find "$(BACKUP_DIR)" -maxdepth 1 -type f -name 'iclaw-backup-*.tar.gz' 2>/dev/null | wc -l) -gt 1 ]; then \
	  echo "❌ 发现多个备份文件，请用 make restore FILE=xxx.tar.gz 指定："; \
	  find "$(BACKUP_DIR)" -maxdepth 1 -type f -name 'iclaw-backup-*.tar.gz' -print; \
	  exit 1; \
	else \
	  echo "❌ 未找到备份文件，请将 iclaw-backup-*.tar.gz 放到当前目录"; \
	  exit 1; \
	fi; \
	if [ ! -f "$$BACKUP" ]; then \
	  echo "❌ 备份文件不存在：$$BACKUP"; \

...
const AgentProfilesPage = lazy(() =>
  import('./AgentProfilesPage').then((m) => ({
    default: m.AgentProfilesPage,
  })),
);
const CapabilitiesPage = lazy(() =>
  import('./CapabilitiesPage').then((m) => ({
...
export function SettingsPage() {
  ...
  const capabilitiesSectionParam = searchParams.get('section') ?? 'skills';
  const capabilitiesSection = ['skills', 'mcp', 'plugins'].includes(
    capabilitiesSectionParam,
  )
    ? capabilitiesSectionParam
    : 'skills';
  const handleCapabilitiesSectionChange = useCallback(
    (section: string) => {
      const nextSearchParams = new URLSearchParams(searchParams);
      nextSearchParams.set('tab', 'capabilities');
            ...
            {activeTab === 'capabilities' && (
              <Suspense fallback={null}>
                <CapabilitiesPage
                  ...
                  onSectionChange={handleCapabilitiesSectionChange}
                />
              </Suspense>
            )}
            {activeTab === 'tasks' && (

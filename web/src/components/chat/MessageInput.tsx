...
export function MessageInput({
  ...
  const handleSend = async (modeOverride?: FollowUpMode) => {
    ...
    if (!trimmed && !hasPending && !hasImages && !hasDocuments) return;
    ...
    if (ok) {
      ...
      if (hasDocuments) setPendingDocuments([]);
    } else {
  ...
  const uploadResumeFile = async (file: File): Promise<PendingDocument> => {
    if (file.size > MAX_DOCUMENT_SIZE_BYTES) {
      throw new Error(
        `简历 ${file.name} 超过 10MB 限制 (${(file.size / 1024 / 1024).toFixed(1)}MB)`,
      );
    }
    ...
    return {
      ...
      mimeType: result.mime_type || file.type || undefined,
      sizeBytes: file.size,
    };
  };
    ...
    if (fileList && fileList.length > 0) {
      ...
      if (resumeFiles.length > 0) {
        const uploads = await Promise.allSettled(
          resumeFiles.map((file) => uploadResumeFile(file)),
        );
        const uploaded = uploads
          .filter(
            (result): result is PromiseFulfilledResult<PendingDocument> =>
        ...
        if (failed) setSendError(`${failed} 个简历上传失败`);
      }
          ...
          {pendingDocuments.length > 0 && (
            <div className="px-3 pt-2.5 pb-1 border-b border-border">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-[11px] text-muted-foreground">
                  已添加 {pendingDocuments.length} 份简历附件，可用于临时分析
                </span>
              </div>
              <div className="flex flex-wrap gap-1 pb-1.5">
                {pendingDocuments.map((doc, i) => (
                  <span
                    ...
                    className="inline-flex items-center gap-1 max-w-[220px] px-2 py-0.5 bg-brand-50 text-primary text-[11px] rounded-md"
                  >
                    <span className="truncate">{doc.name}</span>
                    <button
                      onClick={() => removePendingDocument(i)}
                      className="flex-shrink-0 hover:text-primary cursor-pointer"
                      aria-label="移除简历附件"
                    >
                      <X className="w-3 h-3" />
                    </button>

...
export function ChatView({ groupJid, onBack, headerLeft }: ChatViewProps) {
  ...
  const updateHrAgentEnabled = useChatStore((s) => s.updateHrAgentEnabled);
  ...
  return (
    ...
    >
      ...
      <WorkspaceInteractionModeDialog
        ...
        currentHrAgentEnabled={group?.hr_agent_enabled !== false}
        onClose={() => setShowInteractionModeDialog(false)}
        onSave={(mode) => updateInteractionMode(groupJid, mode)}
        onSaveHrAgentEnabled={(enabled) =>
          updateHrAgentEnabled(groupJid, enabled)
        }
      />

  ...
  return (
    <TooltipProvider delayDuration={200}>
        ...
        <nav className="w-[4.5rem] h-full bg-muted/30 flex flex-col items-center py-3 gap-1 flex-shrink-0">
          ...
          {navItems.map(({ path, icon: Icon, label }) => {
            ...
            const activeClass = isActive
              ? 'bg-gradient-to-br from-brand-500 to-brand-600 text-white shadow-[0_8px_18px_rgba(37,99,235,0.22)]'
      ...
      <ConfirmDialog
        ...
        title="重建工作区"
        message={`确认重建「${clearState.name}」？会永久删除全部聊天记录、上下文、所有子对话及其消息、工作目录文件，以及该工作区的全部 Memory（含版本历史、遗忘记录和 iclaw 称呼偏好）；Home 还会重置首次唤醒状态。关联定时任务会停止并移入回收站，运行历史与持久化目录 (data/extra/) 保留。不可撤销。`}
        confirmText="确认重建"

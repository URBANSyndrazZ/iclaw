...
export function MainAgentCapabilitiesSection() {
  ...
  const persistedHostPolicy = profile
    ...
    : null;
  const managedSkillsError = skillSelectionError(' iclaw Skill', {
    mode: skillsMode,
    ids: skillIds,
  });
  ...
  const save = async () => {
    ...
    try {
      ...
      await loadProfiles();
      toast.success('主 iclaw 模型、推理档位与能力已保存');
    } catch (error) {
    ...
    <section className="space-y-5 border-b border-border py-6">
      <div>
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">
          为主智能体选择完整模型网关环境，并按来源控制 Skills 与 iclaw 附加的
      ...
      <div className="max-w-xl space-y-2">
        ...
        <Select value={modelConfigId} onValueChange={setModelConfigId}>
          <SelectTrigger aria-label="主 iclaw 模型配置">
      ...
      <div className="max-w-xl space-y-2">
        ...
        >
          <SelectTrigger aria-label="主 iclaw 推理努力档位">

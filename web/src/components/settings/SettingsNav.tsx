...
const advancedLinkItems: AdvancedLinkItem[] = [
  {
    key: 'agent-profiles',
    label: '智能体',
    icon: <Bot className="size-4" />,
  ...
  { key: 'tasks', label: '任务', icon: <Clock4 className="size-4" /> },
  { key: 'usage', label: '用量', icon: <BarChart3 className="size-4" /> },
  {
    key: 'billing',
    label: '账单',
    icon: <CreditCard className="size-4" />,
...
export function SettingsNav({
  ...
  const advancedLinks = advancedLinkItems.filter(
    (item) => !item.requiresBilling || canManageBilling,
  );
  const sections = [

...
export type ColorScheme = 'blue' | 'default' | 'orange' | 'neutral';
export type FontStyle = 'default' | 'anthropic';

const THEME_KEY = 'iclaw-theme';
const SCHEME_KEY = 'iclaw-color-scheme';
const FONT_KEY = 'iclaw-font-style';
const LEGACY_THEME_KEY = 'iclaw-theme';
const LEGACY_SCHEME_KEY = 'iclaw-color-scheme';
const LEGACY_FONT_KEY = 'iclaw-font-style';
...
function syncMetaThemeColor() {
  ...
  if (isDark) {
    meta.setAttribute('content', isNeutral ? '#09090b' : isBlue ? '#0b1220' : '#0f172a');
  } else {
    meta.setAttribute(
      'content',
      isOrange ? '#FAF9F5' : isNeutral ? '#ffffff' : isBlue ? '#f8fafc' : '#ffffff',
    );
  }
}
...
function applyColorScheme(scheme: ColorScheme) {
  if (typeof document === 'undefined') return;
  document.documentElement.classList.toggle('theme-blue', scheme === 'blue');
...
export function useTheme() {
  ...
  const colorScheme = useSyncExternalStore(subscribe, readColorScheme, () => 'blue' as ColorScheme);
  ...
  const setColorScheme = useCallback((s: ColorScheme) => {
    if (s === 'blue') window.localStorage.removeItem(SCHEME_KEY);

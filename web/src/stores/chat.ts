...
interface ChatState {
  ...
  updateHrAgentEnabled: (jid: string, enabled: boolean) => Promise<boolean>;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  updateHrAgentEnabled: async (jid: string, enabled: boolean) => {
    try {
      await api.patch<{ success: boolean }>(
        `/api/groups/${encodeURIComponent(jid)}`,
        { hr_agent_enabled: enabled },
      );
      const patchGroups = (groups: Record<string, GroupInfo>) => {
        const group = groups[jid];
        if (!group) return groups;
        return {
          ...groups,
          [jid]: { ...group, hr_agent_enabled: enabled },
        };
      };
      set((state) => ({
        groups: patchGroups(state.groups),
  ...
  handleStreamEvent: (chatJid, event, agentId, runId) => {
    ...
    if (event.eventType === 'task_notification' && event.taskId) {
      ...
      if (
        ...
        notifyIfHidden(`iclaw: ${desc} ${status}`, event.taskSummary);
      }
  ...
  loadAgents: async (jid, opts) => {
    ...
    try {
      ...
      set((s) => {
        ...
        try {
          const savedOrder = localStorage.getItem(`iclaw-agent-order-${jid}`);
          if (savedOrder) {
  ...
  sendAgentMessage: async (
    ...
    try {
      ...
      // HTTP is the durability barrier; do not leave the conversation empty if
      // the matching WebSocket frame is delayed or dropped. WS delivery is
      // deduplicated below by the server-issued message ID.
      const authState = useAuthStore.getState();
      const virtualChatJid = `${jid}#agent:${agentId}`;
      const userMessage: Message = {
        id: data.messageId,
        chat_jid: virtualChatJid,
        sender: authState.user?.id || 'web-user',
        sender_name:
          authState.user?.display_name || authState.user?.username || 'Web',
        content,
        timestamp: data.timestamp,
        ...
        delivery_mode:
          data.disposition === 'queued'
            ...
            : data.disposition === 'steered'
              ? 'steer'
              : null,
        delivery_status: data.disposition === 'started' ? null : 'queued',
        delivery_run_id: data.runId ?? null,
        delivery_updated_at:
          data.disposition === 'started' ? null : data.timestamp,
      };
      ...
      set((s) => {
        const existing = s.agentMessages[agentId] || [];
        const merged = mergeMessagesChronologically(existing, [userMessage]);
        snapshotMessages = merged;
        const currentAgents = s.agents[jid];
        const nextAgents = currentAgents?.map((agent) =>
          agent.id === agentId
            ? {
                ...agent,
                last_active_at: userMessage.timestamp,
                latest_message: {
        ...
        return {
          ...
          agentHasMore: {
            ...s.agentHasMore,
            [agentId]: s.agentHasMore[agentId] ?? false,
          },
          ...(nextAgents ? { agents: { ...s.agents, [jid]: nextAgents } } : {}),
        };
      });
      if (snapshotMessages) {
        void saveAgentMessageSnapshot(
          ...
          agentId,
          snapshotMessages,
          get().agentHasMore[agentId] ?? false,
        );
      }
    } catch (err) {
      ...
      const statusSuffix =
        typeof apiError?.status === 'number' && apiError.status > 0
          ? ` (${apiError.status})`
          : '';
      const detail = `${message}${statusSuffix}`;

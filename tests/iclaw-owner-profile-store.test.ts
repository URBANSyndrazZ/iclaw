...
const root = fs.mkdtempSync(
  path.join(os.tmpdir(), 'iclaw-owner-profile-store-'),
);
...
function reservedRows(raw: Database.Database, workspaceJid: string) {
  return raw
    ...
    .all(
      workspaceJid,
      memoryStore.iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY,
    ) as Array<{
    id: string;
    content: string;
    status: string;
...
describe('iclaw owner preferred-address facade', () => {
  test('set is no-op aware, updates with CAS, clears, and restores without losing history', () => {
    ...
    const empty = ownerProfile.geticlawOwnerProfileProjection(workspaceJid);
    ...
    const created = ownerProfile.seticlawOwnerPreferredAddress({
      workspaceJid,
      preferredAddress: '小何',
      expectedRevision: 0,
    ...
    const noOp = ownerProfile.seticlawOwnerPreferredAddress({
      workspaceJid,
      preferredAddress: '  小何  ',
      expectedRevision: 1,
      context: mutationContext(),
    ...
    const updated = ownerProfile.seticlawOwnerPreferredAddress({
      workspaceJid,
    ...
    const cleared = ownerProfile.cleariclawOwnerPreferredAddress({
      workspaceJid,
      expectedRevision: 2,
      context: mutationContext(),
    });
    ...
    const postClearClaim = ownerProfile.claimiclawOwnerIntroduction({
      workspaceJid,
    ...
    const restored = ownerProfile.seticlawOwnerPreferredAddress({
      workspaceJid,
      preferredAddress: '老何',
      expectedRevision: 3,
      context: mutationContext(),
  ...
  test('stale set and clear compare-and-swap attempts preserve the winner', () => {
    ...
    ownerProfile.seticlawOwnerPreferredAddress({
      workspaceJid,
    ...
    for (const operation of [
      ...
      () =>
        ownerProfile.cleariclawOwnerPreferredAddress({
          workspaceJid,
          expectedRevision: 1,
          context: mutationContext(),
        }),
    ...
    expect(
      ownerProfile.geticlawOwnerProfileProjection(workspaceJid),
  ...
  test('replays a retried first set without creating another revision', () => {
    ...
    const first = ownerProfile.seticlawOwnerPreferredAddress(request);
    const replay = ownerProfile.seticlawOwnerPreferredAddress(request);
  ...
  test('projection is read-through on both cold and already-warm callers', () => {
    ...
    const cold = ownerProfile.geticlawOwnerProfileProjection(workspaceJid);
...
describe('iclaw owner introduction onboarding state', () => {
  test('claims first wake without consuming it until a fenced acknowledgement', () => {
    ...
    const first = ownerProfile.claimiclawOwnerIntroduction({
      workspaceJid,
    ...
    const sameRunnerBeforeAck = ownerProfile.claimiclawOwnerIntroduction({
      workspaceJid,
      leaseOwner: 'session-a',
      leaseMs: 60_000,
    ...
    const contending = ownerProfile.claimiclawOwnerIntroduction({
      workspaceJid,
    ...
    const reclaimed = ownerProfile.claimiclawOwnerIntroduction({
      workspaceJid,
      leaseOwner: 'session-b',
      leaseMs: 60_000,
    ...
    expect(() =>
      ownerProfile.acknowledgeiclawOwnerIntroduction({
        workspaceJid,
        leaseOwner: 'session-a',
    ...
    const acknowledged = ownerProfile.acknowledgeiclawOwnerIntroduction({
      workspaceJid,
      leaseOwner: 'session-b',
      leaseToken: reclaimed.leaseToken!,
      now: '2026-07-28T12:01:17.000Z',
    ...
    const skipped = ownerProfile.skipiclawOwnerIntroduction({
      workspaceJid,
    ...
    expect(
      ownerProfile.claimiclawOwnerIntroduction({
        workspaceJid,
  ...
  test('runner exit releases an unacknowledged lease without consuming first wake', () => {
    ...
    expect(
      ownerProfile.releaseiclawOwnerIntroductionLease(
        'failed-runner',
        '2026-07-28T12:00:01.000Z',
      ),
    ).toBe(1);
    const retry = ownerProfile.claimiclawOwnerIntroduction({
      workspaceJid,
  ...
  test('normal database restart does not consume an unacknowledged first wake', () => {
    ...
    const resumed = ownerProfile.claimiclawOwnerIntroduction({
      workspaceJid,
      leaseOwner: 'restart-runner',
      leaseMs: 60_000,
      now: '2026-07-28T12:00:30.000Z',
  ...
  test('backfills first wake for pre-release v66 claimed rows without consuming it for fresh pending rows', () => {
    ...
    const original = ownerProfile.claimiclawOwnerIntroduction({
    ...
    legacy
      ...
      .run(
        ...
        ownerProfile.iclaw_OWNER_INTRODUCTION_FLOW_KEY,
      );
    legacy.exec(
    ...
    const historicalProjection =
      ownerProfile.geticlawOwnerProfileProjection(historicalWorkspaceJid);
    ...
    const fresh = ownerProfile.claimiclawOwnerIntroduction({
...
describe('v66 owner-profile migration boundary', () => {
  test('does not mistake an address containing a legacy sentinel phrase for the sentinel itself', () => {
    ...
    raw
      ...
      .run(workspaceJid, ownerProfile.iclaw_OWNER_INTRODUCTION_FLOW_KEY);
    raw
      .prepare(
        `UPDATE router_state
         SET value = '65'
  ...
  test('matches the exact legacy skip sentinel once and preserves skipped across later starts', () => {
    ...
    const onceOnlySnapshot = {
      ...
      onboarding: migrated
        ...
        .get(workspaceJid, ownerProfile.iclaw_OWNER_INTRODUCTION_FLOW_KEY),
      artifacts: migrated
        .prepare(
          `SELECT
            (SELECT COUNT(*) FROM workspace_memory_versions WHERE item_id = ?) AS versions,
...
describe('reserved canonical-key integrity', () => {
  test('generic memory mutations fail closed for the owner-profile key', () => {
    ...
    const dedicated = ownerProfile.seticlawOwnerPreferredAddress({
      workspaceJid,

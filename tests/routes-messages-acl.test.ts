...
const SHARED_TMP =
  process.env.iclaw_TEST_DATA_DIR ??
  (() => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'iclaw-routes-messages-'));
    process.env.iclaw_TEST_DATA_DIR = d;
    return d;
  })();
...
vi.mock('../src/config.js', async (importOriginal) => {
  ...
  const dataDir = process.env.iclaw_TEST_DATA_DIR!;
  return {
    ...real,
    DATA_DIR: dataDir,
...
vi.mock('../src/middleware/auth.ts', async (importOriginal) => {
  ...
  return {
    ...
    authMiddleware: async (c: any, next: any) => {
      c.set('user', {
        id: process.env.iclaw_TEST_USER_ID ?? 'alice',
        username: 'alice',
        display_name: 'Alice',
        role: (process.env.iclaw_TEST_USER_ROLE ?? 'member') as
          | 'admin'
          | 'member',
        status: (process.env.iclaw_TEST_USER_STATUS ?? 'active') as
          | 'active'
          | 'disabled'
          | 'deleted',
        permissions: [],
...
function seedTestGroup(
  executionMode: 'host' | 'container' = 'container',
): void {
...
function asUser(
  ...
  process.env.iclaw_TEST_USER_ID = userId;
  process.env.iclaw_TEST_USER_ROLE = role;
  process.env.iclaw_TEST_USER_STATUS = status;
}
...
afterEach(() => {
  delete process.env.iclaw_TEST_USER_ID;
  delete process.env.iclaw_TEST_USER_ROLE;
  delete process.env.iclaw_TEST_USER_STATUS;
...
describe('POST /api/messages — host workspace ACL', () => {
  test('allows an active member owner when single-host mode is enabled', async () => {
    seedTestGroup('host');
    asUser(OWNER_ID, 'member', 'active');
    process.env.ICLAW_SINGLE_HOST_MODE = 'true';
    activeQueryId = 'run-host';
  ...
  test('denies a disabled member even when single-host mode is enabled', async () => {
    seedTestGroup('host');
    asUser(OWNER_ID, 'member', 'disabled');
    process.env.ICLAW_SINGLE_HOST_MODE = 'true';
  ...
  test('denies an active member when single-host mode is disabled', async () => {
    seedTestGroup('host');
    asUser(OWNER_ID, 'member', 'active');

    const { status, body } = await postMessage({
      chatJid: GROUP_JID,
      content: 'container mode is required',
    });

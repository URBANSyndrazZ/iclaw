...
const TEST_ROOT =
  process.env.iclaw_MCP_RUNTIME_TEST_ROOT ??
  (() => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'iclaw-mcp-runtime-boundary-'),
    );
    process.env.iclaw_MCP_RUNTIME_TEST_ROOT = root;
    return root;
  })();
...
vi.mock('../src/config.js', async (importOriginal) => {
  ...
  const root = process.env.iclaw_MCP_RUNTIME_TEST_ROOT!;
  return {
    ...real,
    DATA_DIR: root,
...
afterAll(() => {
  ...
  delete process.env.iclaw_MCP_RUNTIME_TEST_ROOT;
});

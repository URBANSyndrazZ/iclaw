...
describe('channel onboarding frontend protocol contract', () => {
  ...
  test('QR onboarding never exposes WeChat or WhatsApp protocol output as inputs', () => {
    ...
    expect(fields).toContain('扫码结果由 iclaw 安全保存');
  ...
  test('authorized QR accounts can open pairing from the connection dialog', () => {
    const manager = read(
      'web/src/components/settings/ChannelAccountsManager.tsx',
    );
    const branchStart = manager.indexOf(
      "{definition.authMode === 'qr_session' ? (",
    );
    const branchEnd = manager.indexOf(') : (', branchStart);
    const qrBranch = manager.slice(branchStart, branchEnd);

    expect(qrBranch).toContain('扫码与连接状态');
    expect(qrBranch).toContain('supportsChannelPairing(account.provider)');
    expect(qrBranch).toContain('account.has_credentials');
    expect(qrBranch).toContain('<AccountPairingSection account={account} />');
    expect(
      manager.match(/<AccountPairingSection account={account} \/>/g),
    ).toHaveLength(2);
  });

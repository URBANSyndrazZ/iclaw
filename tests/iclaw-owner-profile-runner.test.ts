...
import { iclawFirstWakeAcknowledger } from '../container/agent-runner/src/owner-profile-first-wake.js';
import {
  loadiclawOwnerProfileTurnContext,
  rendericlawOwnerProfileBlock,
  ...
  await vi.waitFor(() => {
    ...
    expect(
      files.filter(
        (name) =>
          name.endsWith('.json') &&
          !name.startsWith('iclaw_owner_profile_result_'),
      ),
    ).toHaveLength(1);
  });
  const file = fs
...
function acknowledgeOwnerProfile(
  ...
  fs.writeFileSync(
    path.join(
      root,
      'tasks',
      `iclaw_owner_profile_result_${requestId}.json`,
    ),
    JSON.stringify({
      success: true,
      projection,
...
describe('iclaw Owner Profile runner projection refresh', () => {
  test('cold and warm turns fetch fresh host projections with exact turn correlation', async () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'iclaw-owner-profile-runner-'),
    );
    roots.push(root);
    ...
    const coldPending = fetchiclawOwnerProfileTurn(ctx, 2_000);
    ...
    const warmPending = fetchiclawOwnerProfileTurn(
      ctx,
      2_000,
      'owner-turn-b',
    );
  ...
  test('fails closed without owner admission or an exact current turn', async () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'iclaw-owner-profile-runner-deny-'),
    );
    roots.push(root);
    await expect(
      fetchiclawOwnerProfileTurn(
  ...
  test('uses a signed private IPC operation to acknowledge first wake', async () => {
    const root = fs.mkdtempSync(
      path.join(os.tmpdir(), 'iclaw-owner-profile-runner-ack-'),
    );
    roots.push(root);
    const scope = { groupFolder: 'home-folder' };
    const capability = issueWorkspaceMemoryWriteCapability(
    ...
    const pending = acknowledgeiclawOwnerProfileFirstWake(
      ctx,
      7,
      'owner-turn-a',
      2_000,
    ...
    fs.writeFileSync(
      path.join(
        root,
        'tasks',
        `iclaw_owner_profile_result_${request.request.requestId}.json`,
      ),
  ...
  test('renders profile data safely and reserves just-woke wording for the new lease claimant', async () => {
    ...
    expect(rendericlawOwnerProfileBlock(awaiting)).toContain(
      'first_wake="true"',
    );
    expect(rendericlawOwnerProfileBlock(awaiting)).toContain(
      'single first-wake greeting',
    );
    const resumedAwaiting = rendericlawOwnerProfileBlock({
      ...awaiting,
      firstWake: false,
    });
    ...
    const rendered = rendericlawOwnerProfileBlock(known);
    ...
    const cold = await loadiclawOwnerProfileTurnContext(fetchProjection);
    const warm = await loadiclawOwnerProfileTurnContext(fetchProjection);
      ...
      const result = await guard(
        {
          ...
          cwd: '/tmp',
          tool_name: 'mcp__iclaw__iclaw_owner_profile',
          tool_input: { action },

...
describe('settings information architecture', () => {
  test('keeps account, system, and administration scopes explicit', () => {
    ...
    expect(nav).toContain("label: '关于 iclaw'");
    ...
    expect(nav).toContain("key: 'agent-profiles'");
    expect(nav).toContain("key: 'capabilities'");
    expect(nav).toContain("key: 'tasks'");
    expect(nav).toContain("key: 'usage'");
  });
  ...
  test('keeps advanced capabilities in settings and redirects old routes', () => {
    ...
    expect(app).toContain('to="/settings?tab=agent-profiles"');
    expect(app).toContain('to="/settings?tab=capabilities&section=skills"');
    expect(app).toContain('to="/settings?tab=tasks"');
    expect(app).toContain('to="/settings?tab=usage"');
    expect(app).toContain('requiredPermission="manage_system_config"');
    expect(settings).toContain(
      "skills: '/settings?tab=capabilities&section=skills'",
    );
    expect(settings).toContain(
      "'mcp-servers': '/settings?tab=capabilities&section=mcp'",
    );
    expect(settings).toContain(
      "plugins: '/settings?tab=capabilities&section=plugins'",
    );
    expect(settings).toContain(
      "bindings: '/settings?tab=my-channels&view=bindings'",
    );
    expect(navItems).toContain("label: 'HR 工作台'");
    expect(navItems).toContain("label: 'HR 助手'");
    expect(navItems).toContain("label: '设置'");
    expect(navItems).not.toContain("label: '能力库'");
    expect(navItems).not.toContain("label: '任务'");
  ...
  test('keeps Agent add-ons, project context, and Provider settings in distinct layers', () => {
    ...
    expect(mainCapabilities).toMatch(/iclaw 用户 Skills|iclaw MCP/);

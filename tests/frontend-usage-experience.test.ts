...
describe('usage page product and accessibility surface', () => {
  test('lazy-loads the Recharts route instead of importing it into the main bundle', () => {
    ...
    expect(app).toContain("import('./pages/SettingsPage')");
    expect(settings).toContain('const UsagePage = lazy(() =>');
    expect(settings).toContain("import('./UsagePage')");
    expect(settings).toContain('<UsagePage />');
  });

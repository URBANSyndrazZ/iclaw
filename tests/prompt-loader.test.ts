...
describe('prompts/ files', () => {
  ...
  test('built-in iclaw identity and one-shot owner bootstrap are explicit', () => {
    const identity = fs.readFileSync(
      path.join(PROMPTS_DIR, 'identity.iclaw.md'),
      'utf-8',
    );
    const bootstrap = fs.readFileSync(
      path.join(PROMPTS_DIR, 'bootstrap.iclaw.md'),
      'utf-8',
    );

    expect(identity).toContain('你是 **iclaw**');
    ...
    expect(bootstrap).toContain('iclaw.owner.preferred_address');
    expect(bootstrap).toContain('iclaw_owner_profile');

...
describe('Docker image distribution contract', () => {
  ...
  test('smoke helper exercises the production entrypoint and real HTTP endpoint', () => {
    ...
    expect(smoke).toContain('--env iclaw_HOST_IDENTITY_MODE=host-root');
    expect(smoke).toContain('--env iclaw_HOST_IDENTITY_MODE=direct');
    expect(smoke).toContain('--env "iclaw_HOST_UID=$smoke_host_uid"');
    expect(smoke).toContain('--env "iclaw_HOST_GID=$smoke_host_gid"');
    expect(smoke).toContain('"${smoke_identity_args[@]}"');
    expect(smoke).not.toContain(
      '--env iclaw_HOST_IDENTITY_MODE=virtualized',
    );
    expect(smoke).not.toContain('--env iclaw_HOST_IDENTITY_MODE=rootless');

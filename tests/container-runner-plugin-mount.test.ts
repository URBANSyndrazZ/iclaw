...
const SHARED_TMP =
  ...
  (() => {
    const d = fs.mkdtempSync(path.join(os.tmpdir(), 'iclaw-cr-mount-'));
...
describe('buildVolumeMounts — AgentProfile runtime policy', () => {
  test('does not write retired Agent tool restrictions into the container env', () => {
    ...
    expect(envContent).not.toContain('iclaw_AGENT_DISALLOWED_TOOLS=');
    expect(envContent).not.toContain('iclaw_AGENT_TOOL_POLICY=');
  });
  ...
  test('host MCP env replaces inherited servers and clears disabled policy', () => {
    const env = {
      iclaw_USER_MCP_SERVERS_JSON: JSON.stringify({ stale: {} }),
      KEEP_ME: 'yes',
    };

    replaceHostMcpServersEnv(env, { github: { command: 'github-mcp' } });
    expect(JSON.parse(env.iclaw_USER_MCP_SERVERS_JSON)).toEqual({
      github: { command: 'github-mcp' },
    });
    expect(env.KEEP_ME).toBe('yes');

    replaceHostMcpServersEnv(env, {});
    expect(env.iclaw_USER_MCP_SERVERS_JSON).toBeUndefined();
    expect(env.KEEP_ME).toBe('yes');
  });

...
const SHARED_TMP =
  process.env.iclaw_HOST_MOUNT_RUNNER_TEST_DIR ??
  (() => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'iclaw-host-mount-runner-'),
    );
    process.env.iclaw_HOST_MOUNT_RUNNER_TEST_DIR = dir;
    return dir;
  })();

vi.mock('../src/config.js', async (importOriginal) => {
  const real = (await importOriginal()) as Record<string, unknown>;
  const root = process.env.iclaw_HOST_MOUNT_RUNNER_TEST_DIR!;
  return {
    ...real,

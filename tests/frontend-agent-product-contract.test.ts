...
describe('Agent-first frontend product contracts', () => {
  ...
  it('shows the legacy built-in Agent name as iclaw', () => {
    expect(getAgentProfileDisplayName('Default Agent')).toBe('iclaw');
    expect(getAgentProfileDisplayName(undefined)).toBe('iclaw');
  ...
  it('keeps the default iclaw Agent first and retains its internal home context', () => {
    const home = workspace('web:main', 'agent-iclaw', 'iclaw');
    home.is_my_home = true;

    const sections = groupWorkspacesByAgent(
      [
        workspace('web:review', 'agent-reviewer', '代码审查员'),
        workspace('web:project', 'agent-iclaw', 'iclaw'),
        home,
      ],
      'agent-iclaw',
    );
  ...
  it('keeps the home context separate from additional Agent workspaces for navigation', () => {
    ...
    const [section] = groupWorkspacesByAgent(
      [workspace('web:project', 'agent-iclaw', 'iclaw'), home],
      'agent-iclaw',
    );
  ...
  it('presents the home context as the named main workspace of the primary Agent', () => {
    ...
    expect(
      ...
      { jid: 'web:main', name: 'iclaw' },
      { jid: 'web:project', name: 'web:project' },
    ]);
  });

  it('separates iclaw as the primary Agent from custom Agents', () => {
    const sections = groupWorkspacesByAgent(
      [
        workspace('web:main', 'agent-iclaw', 'iclaw'),
        workspace('web:review', 'agent-reviewer', '代码审查员'),
      ],
      'agent-iclaw',
    );
    ...
    expect(partitioned.primary?.name).toBe('iclaw');
    expect(partitioned.custom.map((section) => section.name)).toEqual([
      '代码审查员',
    ]);
  });

  it('keeps the primary iclaw Agent fixed open while custom Agents remain collapsible', () => {

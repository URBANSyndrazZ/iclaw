...
describe('iclaw Owner Profile exact active-turn authorization', () => {
...
describe('iclaw Owner Profile recently completed first-wake authorization', () => {
  const turn = {
    chatJid: 'web:home#agent:conversation-1',
    messageId: 'owner-turn',
    runtimeTurnId: 'owner-turn',
    scheduledTaskId: null,
  };

  test('acknowledges the exact owner turn after clearCompleted', () => {
    const registry = new AgentBuilderTurnRegistry();
    registry.startBatch('home', [turn]);
    registry.clearCompleted('home', [
      { chatJid: turn.chatJid, messageId: turn.messageId },
    ]);
    ...
    expect(
      registry.requireExactRecentlyCompletedOwnerHumanTurn(
        'home',
        turn.runtimeTurnId,
        () => ownerWebInput,
        () => true,
      ...
      messageId: 'owner-turn',
      content: ownerWebInput.content,
    });
  });

  test('fails closed for non-owner, stale, or reset completed turns', () => {
    const registry = new AgentBuilderTurnRegistry();
    registry.startBatch('home', [turn]);
    registry.clearCompleted('home', [
      { chatJid: turn.chatJid, messageId: turn.messageId },
    ...
    ).toThrow('acknowledgement is unavailable');
    ...
    ).toThrow('exact recently completed conversation turn');

    registry.delete('home');
    expect(() =>
      registry.requireExactRecentlyCompletedOwnerHumanTurn(
        'home',
        turn.runtimeTurnId,
...
describe('iclaw Owner Profile actual sender identity', () => {
...
describe('iclaw Owner Profile home-anchored sender identity', () => {
...
describe('iclaw Owner Profile runtime isolation', () => {
  const base = {
  ...
  test('allows the default iclaw main and same-workspace conversation sessions', () => {

...
describe('Run Now idempotency key lifecycle', () => {
  ...
  test('expires stale persisted keys after 24 hours', () => {
    ...
    const repaired = JSON.parse(
      values.get('iclaw:task-run-idempotency:user-1')!,
    );
  ...
  test.each([
    ...
    const values = new Map<string, string>([
      ['iclaw:task-run-idempotency:user-1', raw],
      [
        'iclaw:task-run-idempotency:user-2',
        JSON.stringify({
    ...
    expect(
      JSON.parse(values.get('iclaw:task-run-idempotency:user-1')!),
    ).toEqual({
      'task-1': {
    ...
    expect(
      JSON.parse(values.get('iclaw:task-run-idempotency:user-2')!),
    ).toEqual({
      untouched: { key: 'other-user-key', createdAt: 900 },
    });
  });
  ...
  test('keeps valid entries while removing malformed and expired siblings', () => {
    const storage = 'iclaw:task-run-idempotency:user-1';
    const values = new Map<string, string>([
      [
        storage,
        JSON.stringify({

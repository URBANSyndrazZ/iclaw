...
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    c.set('user', {
      id: process.env.iclaw_MEMORY_TEST_USER ?? 'alice',
      username: process.env.iclaw_MEMORY_TEST_USER ?? 'alice',
      display_name: '',
      role: process.env.iclaw_MEMORY_TEST_ROLE ?? 'member',
      status: 'active',
      permissions: [],
...
function asUser(id: string, role: 'member' | 'admin' = 'member'): void {
  process.env.iclaw_MEMORY_TEST_USER = id;
  process.env.iclaw_MEMORY_TEST_ROLE = role;
}
...
afterAll(() => {
  delete process.env.iclaw_MEMORY_TEST_USER;
  delete process.env.iclaw_MEMORY_TEST_ROLE;
  db.closeDatabase();

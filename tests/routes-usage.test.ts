...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'iclaw-routes-usage-'));
...
describe('/api/usage contract and isolation', () => {
  ...
  test('main iclaw attribution has a filterable stable sentinel', async () => {
    ...
    expect(body.attributions.agents).toEqual([
      expect.objectContaining({ key: '__main__', name: 'iclaw' }),
    ]);
  });

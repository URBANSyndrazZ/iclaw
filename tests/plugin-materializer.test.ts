...
beforeEach(() => {
  tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'iclaw-mat-'));
});
...
describe('materializeUserRuntime', () => {
  ...
  test('builds runtime tree from catalog snapshot with isolated inodes', () => {
    ...
    expect(
      fs.existsSync(path.join(rtDir, '@iclaw-runtime-markers')),
    ).toBe(false);
    const markerPath = path.join(
      getUserSnapshotsDir(USER), 'sha256-aaa', '@iclaw-runtime-markers', 'mp1', 'p1.json',
    );

...
const root = fs.mkdtempSync(
  path.join(os.tmpdir(), 'routes-iclaw-owner-profile-'),
);
...
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    const id = process.env.iclaw_OWNER_PROFILE_ROUTE_USER ?? 'route-owner';
    c.set('user', {
      id,
      username: id,
      display_name: id,
      role:
        process.env.iclaw_OWNER_PROFILE_ROUTE_ROLE === 'admin'
          ? 'admin'
          : 'member',
      status: 'active',
      permissions: [],
...
function asUser(id: string, role: 'member' | 'admin' = 'member'): void {
  process.env.iclaw_OWNER_PROFILE_ROUTE_USER = id;
  process.env.iclaw_OWNER_PROFILE_ROUTE_ROLE = role;
}
...
afterEach(() => {
  delete process.env.iclaw_OWNER_PROFILE_ROUTE_USER;
  delete process.env.iclaw_OWNER_PROFILE_ROUTE_ROLE;
});
...
describe('/api/workspaces/:jid/owner-profile', () => {
  ...
  test('non-Home and custom-profile Home workspaces fail closed', async () => {
    ...
    const custom = db.createAgentProfile({
      ...
      identityPrompt: 'Remain isolated from iclaw Owner Profile.',
    });
  ...
  test('generic Workspace Memory API cannot create, update, or forget the reserved item', async () => {
    ...
    const genericCreate = await genericMemoryRequest(skipHomeJid, '', 'POST', {
      ...
      canonicalKey: memoryStore.iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY,
    });

...
const SHARED_TMP =
  process.env.iclaw_HOST_MOUNT_ROUTE_TEST_DIR ??
  (() => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'iclaw-host-mount-route-'),
    );
    process.env.iclaw_HOST_MOUNT_ROUTE_TEST_DIR = dir;
    return dir;
  })();

vi.mock('../src/config.js', async (importOriginal) => {
  const real = (await importOriginal()) as Record<string, unknown>;
  const root = process.env.iclaw_HOST_MOUNT_ROUTE_TEST_DIR!;
  return {
    ...real,
...
vi.mock('../src/middleware/auth.ts', () => ({
  authMiddleware: async (c: any, next: any) => {
    c.set('user', {
      id: process.env.iclaw_TEST_USER_ID ?? 'route-admin',
      username: process.env.iclaw_TEST_USER_ID ?? 'route-admin',
      role: process.env.iclaw_TEST_USER_ROLE ?? 'admin',
      status: process.env.iclaw_TEST_USER_STATUS ?? 'active',
      permissions: [],
    });
    return next();
  },
...
function asUser(
  ...
  process.env.iclaw_TEST_USER_ID = id;

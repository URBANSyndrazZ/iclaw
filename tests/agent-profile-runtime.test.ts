...
describe('AgentProfile runtime invalidation', () => {
  test('default iclaw takes its compact threshold from global policy', () => {
    ...
    expect(effective?.identity_hash).toBe(
      runtime.bindiclawPlatformIdentityHash(
  ...
  test('custom Agents do not inherit the protected iclaw identity hash', () => {
    ...
    expect(effective.identity_hash).not.toBe(
      runtime.bindiclawPlatformIdentityHash(effective.identity_hash),
    );
  });

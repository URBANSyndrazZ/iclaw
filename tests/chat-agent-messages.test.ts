...
describe('sendAgentMessage optimistic rendering', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('renders the acknowledged user message immediately and deduplicates its WS frame', async () => {
    const jid = 'web:main';
    const agentId = 'agent-1';
    ...
    await expect(
      useChatStore
        .getState()
        .sendAgentMessage(jid, agentId, '你好', undefined, 'queue'),
    ).resolves.toBe(true);

    const state = useChatStore.getState();
    expect(state.agentMessages[agentId]).toHaveLength(1);
    expect(state.agentMessages[agentId][0]).toMatchObject({
      id: 'user-message-1',
    ...
    expect(useChatStore.getState().agentMessages[agentId]).toHaveLength(1);
  });
});

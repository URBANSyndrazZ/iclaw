...
const TEST_ROOT =
  process.env.iclaw_HOST_MOUNT_BROWSE_TEST_DIR ??
  (() => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'iclaw-host-mount-browse-'),
    );
    process.env.iclaw_HOST_MOUNT_BROWSE_TEST_DIR = dir;
    return dir;
  })();
...
const protectedDataDir = path.join(homeLikeRoot, 'iclaw-data');

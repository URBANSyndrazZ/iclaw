...
describe('account-scoped channel pairing', () => {
  test('codes are independent between accounts, rotate per account, and are single-use', () => {
    ...
    expect(
      pairing.inspectPairingCode(first.code, {
        userId: 'other-user',
        accountId: 'bot-a',
      }),
    ...
    expect(
      ...
      ok: true,
      claims: { userId: 'pair-owner', accountId: 'bot-a' },
    });
  ...
  test('inspection preserves a code and persisted codes survive database restart', () => {
    const code = pairing.generatePairingCode('pair-owner', 'restart-a').code;
    const expected = { userId: 'pair-owner', accountId: 'restart-a' };
    expect(
      pairing.inspectPairingCode(code, {
        userId: expected.userId,
        accountId: expected.accountId,
      }),
    ).toEqual({ ok: true, claims: expected });

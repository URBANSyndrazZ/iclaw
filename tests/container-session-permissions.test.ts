...
const testRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), 'iclaw-container-permissions-'),
);
...
vi.mock('../src/config.js', async (importOriginal) => {
  ...
  return {
    ...
    CONTAINER_IMAGE: 'iclaw-agent:test',
    TIMEZONE: 'UTC',
  };
});
...
describe('buildContainerArgs identity contract', () => {
  test('passes independently validated non-root uid and gid in direct mode', () => {
    ...
    expect(envArgs(args)).toEqual(
      expect.arrayContaining([
        'iclaw_HOST_IDENTITY_MODE=direct',
        'iclaw_HOST_UID=1002',
        'iclaw_HOST_GID=1234',
      ]),
    );
  ...
  test.each([
    ...
    expect(envArgs(args)).toContain(`iclaw_HOST_IDENTITY_MODE=${mode}`);
    expect(
      envArgs(args).some((arg) => arg.startsWith('iclaw_HOST_UID=')),
    ).toBe(false);
    expect(
      envArgs(args).some((arg) => arg.startsWith('iclaw_HOST_GID=')),
    ).toBe(false);
  });
});
...
describe('entrypoint permission contract', () => {
  ...
  test('uses fixed root-owned helpers and shadows stale malicious env values', () => {
    ...
    expect(entrypoint).toContain('iclaw_source_runtime_env');
    expect(helper).toContain(
      '/usr/local/bin/node /app/session-permissions-watcher.mjs',
    );
    expect(helper).not.toContain('iclaw_SESSION_PERMISSION_HELPER:-');
    expect(helper).not.toContain('groupmod --non-unique');
    expect(helper).toContain('local iclaw_SESSION_ROOT=');
    expect(helper).toContain('local iclaw_INTERNAL_WATCHER_PID=');
    expect(entrypoint).toContain(
      'npx tsc --outDir /tmp/dist --incremental false',
    );
    expect(
      entrypoint.indexOf('iclaw_prepare_generated_path dist'),
    ).toBeLessThan(
...
const integrationImage =
  process.env.iclaw_CONTAINER_PERMISSION_TEST_IMAGE ??
  'URBANSyndrazZ/iclaw-agent:latest';
...
describe.skipIf(!integrationImageAvailable)(
  ...
  () => {
    ...
    test('direct mode safely falls back from an image group gid collision', () => {
      expect(
        runHelper(`
          ...
          test -n "$shadow_gid"
          iclaw_HOST_IDENTITY_MODE=direct
          iclaw_HOST_UID=12346
          iclaw_HOST_GID="$shadow_gid"
    ...
    test('rootless watcher upgrades legacy node-owned roots before readiness', () => {
      expect(
        runImageScript(`
          ...
          iclaw_HOST_IDENTITY_MODE=rootless
          iclaw_configure_node_identity
    ...
    test('fake legacy migration marker cannot preserve unsafe credentials', () => {
      expect(
        runImageScript(`
          source /app/session-permissions.sh
          mkdir -p /home/node/.claude /workspace/{group,ipc,extra}
          touch /home/node/.claude/.iclaw-owner-v2-12346-12347
          touch /home/node/.claude/credential
          chown -R 1000:1000 /home/node/.claude
          chmod 0600 /home/node/.claude/.iclaw-owner-v2-12346-12347
    ...
    test('concurrent direct migrations are idempotent without markers', () => {
      expect(
        runImageScript(`
          ...
          test "$(find /home/node/.feishu-cli -name '.iclaw-owner-v2-*' | wc -l)" = 0
    ...
    test('host-root live watcher repairs new root-owned entries privately', () => {
      expect(
        runImageScript(`
          source /app/session-permissions.sh
          mkdir -p /home/node/.claude /workspace/{group,ipc,extra}
          iclaw_HOST_IDENTITY_MODE=host-root
          iclaw_configure_node_identity
    ...
    test('restrictive prompt bind is copied read-only without touching source', () => {
      ...
      const prompt = path.join(prompts, 'identity.iclaw.md');
      ...
      expect(
        runImageScript(
          `
            ...
            test "$(stat -c '%u:%g:%a' /tmp/prompts/identity.iclaw.md)" = \
              '0:0:444'
            setpriv --reuid="$(id -u node)" --regid="$(id -g node)" \
              --clear-groups -- sh -ceu '
                test "$(cat /tmp/prompts/identity.iclaw.md)" = "runtime prompt"
                test ! -w /tmp/prompts/identity.iclaw.md
                test ! -w /app/prompts/identity.iclaw.md
              '
            printf copied
          `,
    ...
    test('runtime env cannot override root-control variables', () => {
      ...
      fs.writeFileSync(
        ...
        [
          "iclaw_HOST_IDENTITY_MODE='unknown'",
          "iclaw_INTERNAL_IDENTITY_MODE='pwned'",
          "iclaw_INTERNAL_WATCHER_PID='1'",
          "iclaw_SESSION_ROOT='/'",
          "iclaw_SESSION_PERMISSION_HELPER='/workspace/group/evil.sh'",
          "PROJECT_ENV='kept'",
        ].join('\n'),
        { mode: 0o600 },
      );
      expect(
        runHelper(
          `
            ...
            iclaw_INTERNAL_WATCHER_PID=4242
            iclaw_source_runtime_env
            printf '%s:%s:%s:%s' \
              "$iclaw_HOST_IDENTITY_MODE" \
              "$iclaw_INTERNAL_IDENTITY_MODE" \
    ...
    test('descriptor bridge resists file-to-symlink swaps', () => {
      expect(
        runHelper(`
            ...
            watcher_pid=$!
            for attempt in $(seq 1 200); do
              [ -e /run/iclaw-session-watcher.ready ] && break
              kill -0 "$watcher_pid"
              sleep 0.02
            done
            ...
            printf '%s:%s:%s' \
              ...
              "$(cat "$outside")" \
              "$([ -e /run/iclaw-session-watcher.failed ] && echo failed || echo safe)"
          `),

...
describe('buildClaudeEnvLines', () => {
  ...
  test('prevents workspace overrides from replacing third-party managed values', () => {
    const lines = buildContainerEnvLines(
      config({ anthropicModel: 'glm-5.2[1m]' }),
      {
        customEnv: {
          ...
          iclaw_FALLBACK_MODEL: 'workspace-fallback',
          PROJECT_ENV: 'kept',
        },
      },
      NO_CUSTOM_ENV,
    ...
    expect(lines).not.toContain('iclaw_FALLBACK_MODEL=workspace-fallback');
  ...
  test('blocks legacy internal workspace path variables from custom env', () => {
    const lines = buildContainerEnvLines(
      ...
      {
        customEnv: {
          iclaw_WORKSPACE_GLOBAL: '/attacker/global',
          iclaw_WORKSPACE_MEMORY: '/attacker/memory',
          PROJECT_ENV: 'kept',
        },
      },
      NO_CUSTOM_ENV,
    ...
    expect(lines).not.toContain('iclaw_WORKSPACE_GLOBAL=/attacker/global');
    expect(lines).not.toContain('iclaw_WORKSPACE_MEMORY=/attacker/memory');
    expect(lines).toContain('PROJECT_ENV=kept');
  });
  ...
  test('blocks container identity and session permission control variables', () => {
    const blocked = {
      ...
      iclaw_INTERNAL_FUTURE_ROOT_KNOB: '/workspace/group/evil',
      iclaw_PASSWD_FILE: '/workspace/group/passwd',
      iclaw_RECONCILE_SESSION_PERMISSIONS: '1',
      iclaw_MOUNT_PREPARE_MODE: 'recursive',
      iclaw_RUNTIME_USER: 'root',
      iclaw_SESSION_ROOT: '/',
      iclaw_SESSION_PERMISSION_PID: '1',
  ...
  test('injects an authoritative endpoint kind that custom env cannot replace', () => {
    const thirdParty = buildContainerEnvLines(
      ...
      { customEnv: { iclaw_CLAUDE_ENDPOINT_KIND: 'official' } },
      { iclaw_CLAUDE_ENDPOINT_KIND: 'official' },
    );
    ...
    expect(thirdParty).toContain('iclaw_CLAUDE_ENDPOINT_KIND=custom');
    expect(thirdParty).not.toContain('iclaw_CLAUDE_ENDPOINT_KIND=official');
    expect(official).toContain('iclaw_CLAUDE_ENDPOINT_KIND=official');
  });
  ...
  test('clears inherited provider values before host-mode config is applied', () => {
    const env: Record<string, string | undefined> = {
      ...
      iclaw_CLAUDE_ENDPOINT_KIND: 'custom',
      KEEP_ME: 'yes',
    };
    ...
    expect(env.iclaw_CLAUDE_ENDPOINT_KIND).toBeUndefined();

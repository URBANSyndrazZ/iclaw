...
describe('AgentProfile DB model', () => {
  test('creates one default AgentProfile for every new user', () => {
    ...
    expect(profiles[0].name).toBe('iclaw');
    ...
    expect(profiles[0].identity_hash).toBe(
      computeAgentProfileIdentityHash('', true, undefined, 'iclaw'),
    );
  });

  test('persists admin iclaw as managed; global host policy is resolved at runtime', () => {
  ...
  test('renames the legacy built-in Agent without changing custom default names', () => {
    ...
    expect(migrated.name).toBe('iclaw');
  ...
  test('keeps the current iclaw default name stable', () => {
  ...
  test('permanently binds Home Workspace to the built-in iclaw', () => {

...
describe('Electron Desktop Shell contract', () => {
  ...
  it('keeps packaging focused on the desktop shell', () => {
    expect(builderConfig).toContain('productName: iclaw');
    expect(builderConfig).toContain('icon: assets/iclaw-icon.png');
    expect(builderConfig).toContain('icon: assets/iclaw.icns');

...
afterAll(() => {
  delete process.env.iclaw_MIGRATION_BACKUP_DIR;
  try {
    db.closeDatabase();
  } catch {
...
describe('database upgrade safety gate', () => {
  test('backs up before destructive migration, preserves audit orphans, and aborts when backup fails', () => {
    ...
    process.env.iclaw_MIGRATION_BACKUP_DIR = migrationBackups;
    db.initDatabase();
    ...
    process.env.iclaw_MIGRATION_BACKUP_DIR = invalidBackupDir;
...
describe('schema version head', () => {
  test('pins the head version so a bump stays a reviewed decision', () => {
    ...
    // v70: makes channel pairing codes durable and hashed at rest; coverage is
    // in tests/channel-account-pairing.test.ts.

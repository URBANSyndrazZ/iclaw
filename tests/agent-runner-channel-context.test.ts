...
describe('Agent Runner channel turn context', () => {
  test('omits HR tools only when the host disables the capability', () => {
    const { context, tools } = setup();
    expect(tools.map((tool) => tool.name)).toContain('hr_list_jobs');

    const disabledTools = createMcpTools({
      ...context,
      hrAgentEnabled: false,
    });
    expect(
      disabledTools
        .map((tool) => tool.name)
        .filter((name) => name.startsWith('hr_')),
    ).toEqual([]);
  });

...
const tmpDir = fs.mkdtempSync(
  path.join(os.tmpdir(), 'iclaw-system-settings-security-'),
);
  ...
  return {
    ...
    authMiddleware: async (c: any, next: any) => {
      c.set('user', {
        ...
        role: process.env.iclaw_TEST_ROLE ?? 'member',
        status: 'active',
        permissions: JSON.parse(process.env.iclaw_TEST_PERMISSIONS ?? '[]'),
...
function asUser(role: 'admin' | 'member', permissions: string[] = []): void {
  process.env.iclaw_TEST_ROLE = role;
  process.env.iclaw_TEST_PERMISSIONS = JSON.stringify(permissions);
}
...
afterAll(() => {
  delete process.env.iclaw_TEST_ROLE;
  delete process.env.iclaw_TEST_PERMISSIONS;

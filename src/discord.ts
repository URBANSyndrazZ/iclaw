...
import type { ChannelPairAttemptResult } from './channel-admission.js';
...
export function createDiscordConnection(
  ...
  async function handleMessage(
    ...
    try {
      ...
      // iclaw (the previous implementation incorrectly keyed them by the
    ...
    async connect(opts: DiscordConnectOpts): Promise<boolean> {
      ...
      try {
        ...
        discordClient.once(Events.ClientReady, async (readyClient) => {
          ...
          // Register iclaw slash commands as Discord Application Commands
          try {
        ...
        discordClient.on(Events.InteractionCreate, async (interaction) => {
          ...
          // Build command string matching iclaw's text command format.

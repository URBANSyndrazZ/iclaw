...
} from './iclaw-bootstrap.js';
import {
  acknowledgeiclawOwnerIntroduction,
  claimiclawOwnerIntroduction,
  cleariclawOwnerPreferredAddress,
  geticlawOwnerProfileProjection,
  OwnerProfileStoreError,
  seticlawOwnerPreferredAddress,
  skipiclawOwnerIntroduction,
  type iclawOwnerProfileProjection,
...
import { consumePairingCode, inspectPairingCode } from './telegram-pairing.js';
...
import { startHrAnalysisQueueWorker } from './hr/analysis-service.js';
import {
  executeHrAgentCapability,
  hrAgentBaseUrl,
  scanExpiredHrJobs,
} from './hr/agent-capability.js';
...
export function feedStreamEventToCard(
  ...
  switch (se.eventType) {
    ...
    case 'prompt_suggestion':
      ...
      break;
    case 'context_audit':
      // Context audits describe iclaw/Claude runtime wiring. Keep them in
...
function resolveiclawOwnerProfileForTurn(input: {
...
}): iclawOwnerProfileProjection | undefined {
  if (
    !isiclawBootstrapTurn({
      turnId: input.turnId,
  ...
  try {
    return geticlawOwnerProfileProjection(workspaceJid);
  } catch {
    return undefined;
  }
}

function isiclawOwnerProfileRuntimeEligible(input: {
  group: RegisteredGroup;
  profile: AgentProfile | undefined;
  turnId?: string;
  isScheduledTask?: boolean;
  ...
  return isiclawOwnerProfileRuntimeStructurallyEligible({
    isHome: input.group.is_home === true,
    isDefaultProfile: input.profile?.is_default === true,
    isScheduledTask: input.isScheduledTask,
...
function bindChannelOutboxScope(
  ...
  return activeChannelOutboxScopes.bind(key, {
    ...
    inputTurnId,
    owner: `iclaw-outbox:${process.pid}:${runtime.runId}`,
  });
}
...
function loadState(): void {
  ...
  for (const [jid, group] of Object.entries(registeredGroups)) {
    ...
    const expectedMode = resolveHomeExecutionMode(owner?.role);
...
/** A paired/registered direct chat is already bound to one iclaw user.
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  if (isRecovery) {
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro:
          '检测到当前 workspace 切换或更新了顶层 AgentProfile 身份提示词，底层模型 session 已重置。以下是 iclaw 保存的最近对话记录，供你在新身份下延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        maxMessageLength: 700,
        intro:
          '检测到本次因切换 provider 需要使用新的底层模型 session。以下是 iclaw 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
  ...
  let output:
    | {
        ...
        error?: string;
        startupFailureTerminal?: boolean;
      }
    | undefined;
  ...
  if (isErrorExit && output.startupFailureTerminal) {
    const errorDetail =
      output.error || lastError || 'Agent runtime startup failed';
    sendSystemMessage(chatJid, 'agent_error', errorDetail);
    await projectCurrentScheduledGroupTerminal('failed', errorDetail);
    logger.error(
      { group: group.name, error: errorDetail },
      'Agent runtime startup failed terminally; skipping retry',
    );
    commitCursor();
    await clearProcessingIndicatorForInput(ipcReplyTurnTracker.inputTurnId);
    return true;
...
async function runAgent(
  ...
  const iclawOwnerProfile = resolveiclawOwnerProfileForTurn({
    group,
    profile: resolvedAgentProfile,
    turnId,
    isScheduledTask: Boolean(messageTaskId),
  });
  const iclawBootstrapPending =
    iclawOwnerProfile?.onboarding.state === 'pending' ||
    iclawOwnerProfile?.onboarding.state === 'claimed';
  const iclawOwnerProfileEnabled = isiclawOwnerProfileRuntimeEligible({
    group,
    profile: resolvedAgentProfile,
    turnId,
    isScheduledTask: Boolean(messageTaskId),
  ...
  try {
    ...
    if (executionMode === 'host') {
      ...
      const currentOwner = group.created_by
        ? getUserById(group.created_by)
        : undefined;
      if (!canRunHostAgent(currentOwner)) {
        logger.warn(
      ...
      output = await runAgentWithModelFallback(
        ...
        group,
        {
          ...
          isHome,
          isAdminHome,
          iclawBootstrapPending,
          iclawOwnerProfile,
          iclawOwnerProfileEnabled,
          agentBuilderEnabled: resolvedAgentProfile?.is_default === true,
          hrAgentEnabled: group.hr_agent_enabled !== false,
          images,
          messageTaskId,
          ...
          try {
            ...
            if (
              ...
              (parsedData.type.endsWith('_task') ||
                parsedData.type.endsWith('_capability'))
            ) {
...
async function processTaskIpc(
  data: {
    ...
    // Dedicated built-in iclaw Owner Profile facade.
    preferredAddress?: string;
    expectedOnboardingRevision?: number;
    leaseToken?: number;
    // For the main iclaw's conversational Agent Builder
    profileId?: string;
    draftId?: string;
  ...
  const requireOwnerProfileActor = () => {
    ...
    const resolveOwnerTurn =
      data.operation === 'get'
        ...
        : data.operation === 'ack_first_wake'
          ? activeAgentBuilderTurns.requireExactRecentlyCompletedOwnerHumanTurn.bind(
              activeAgentBuilderTurns,
            )
  ...
  switch (data.type) {
      ...
      try {
        ...
        switch (data.operation) {
          case 'get': {
            ...
            const claim = claimiclawOwnerIntroduction({
          ...
          case 'ack_first_wake': {
            ...
            result = acknowledgeiclawOwnerIntroduction({
              workspaceJid: principal.workspaceJid,
              leaseOwner: data.runnerInstanceId,
          ...
          case 'set':
            result = seticlawOwnerPreferredAddress({
              workspaceJid: principal.workspaceJid,
          ...
          case 'clear':
            ...
            result = cleariclawOwnerPreferredAddress({
              workspaceJid: principal.workspaceJid,
              expectedRevision: data.expectedRevision!,
              idempotencyKey: data.idempotencyKey,
              context: principal.context,
            ...
            break;
          case 'skip':
            result = skipiclawOwnerIntroduction({
              workspaceJid: principal.workspaceJid,
        ...
        logger.warn(
          ...
          'iclaw Owner Profile IPC request failed',
        );
      }
      break;
    }
    ...
    case 'hr_capability': {
      ...
      const finish = (payload: Record<string, unknown>): void =>
        writeTaskResult(tasksDir, 'hr_capability', data.requestId, payload);
      try {
        if (!data.requestId || !SAFE_REQUEST_ID_RE.test(data.requestId)) {
          throw new Error('Invalid HR capability requestId');
        }
        const ownerId = sourceGroupEntry?.created_by;
        const owner = ownerId ? getUserById(ownerId) : undefined;
        if (!owner || owner.status !== 'active') {
          throw new Error('HR workspace owner is unavailable');
        }
        if (sourceGroupEntry?.hr_agent_enabled === false) {
          throw new Error('HR capability is disabled for this workspace');
        }
        const params =
          data.params &&
        ...
        logger.warn(
          ...
          'HR capability request rejected or failed',
        );
      }
      break;
    }
      ...
      try {
        ...
        if (isFeishuCapabilityMutation(request)) {
          ...
          const mutation = await deliverFeishuCapabilityMutation({
            ...
            request,
            owner: `iclaw-feishu-capability:${process.pid}:${outboxScope.turnRunId}`,
            execute: () =>
    ...
    default:
      ...
      if (
        ...
        (data.type.endsWith('_task') || data.type.endsWith('_capability'))
      ) {
        writeTaskResult(tasksDir, data.type, data.requestId, {
          success: false,
          error: 'This capability is not supported by the running host.',
        });
      }
  }
}
...
async function processAgentConversation(
  ...
  // session was cleared by provider/model switching, inject persisted iclaw
    ...
    historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro: resetForAgentProfile
          ? '检测到当前 workspace 切换或更新了顶层 AgentProfile 身份提示词，当前 agent 的底层模型 session 已重置。以下是 iclaw 保存的最近对话记录，供你在新身份下延续上下文。'
          : '检测到当前 agent 的底层模型 session 是新的（可能因为切换 provider/model 或恢复失败）。以下是 iclaw 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      logger.info(
  ...
  try {
    ...
    const containerInput: ContainerInput = {
      ...
      hrAgentEnabled: effectiveGroup.hr_agent_enabled !== false,
      iclawBootstrapPending:
        iclawOwnerProfile?.onboarding.state === 'pending' ||
        iclawOwnerProfile?.onboarding.state === 'claimed',
      iclawOwnerProfile,
    ...
    if (output.status === 'error' && output.startupFailureTerminal) {
      const detail =
        output.error || lastError || 'Agent runtime startup failed';
      sendSystemMessage(virtualChatJid, 'agent_error', detail);
      agentDeterministicTerminalError = detail;
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          // the notify_channels broadcast (URBANSyndrazZ/iclaw#559).
...
async function ensureDockerRunning(): Promise<void> {
  ...
  // Kill and clean up orphaned iclaw containers from previous runs
  try {
    const { stdout } = await execFileAsync(
      'docker',
      ['ps', '--filter', 'name=iclaw-', '--format', '{{.Names}}'],
      { timeout: 10000 },
    );
 ...
 * by a different iclaw user), re-routing is skipped — the chat stays with
...
function buildOnNewChat(
  ...
  return (chatJid, chatName) => {
    ...
    if (existing) {
      ...
      //      a different iclaw user → do NOT re-route.
      //
      ...
      if (!existing.is_home) {
        ...
        if (previousOwnerStillConnected) {
          ...
          Object.assign(existing, releaseOwner(existing), {
            ...
            // Trust belongs to the previous iclaw user's channel binding,
...
function buildOnPairAttempt(
  ...
  return async (jid, chatName, code) => {
    const codeValidation = inspectPairingCode(code, {
      userId,
      accountId,
      allowLegacyUnscopedCode,
    });
    if (!codeValidation.ok) {
      logger.warn(
        ...
        'Channel pairing code rejected',
      );
      return { ok: false, reason: codeValidation.reason };
    }

    const claims = codeValidation.claims;
    const parsed = parseChannelAddress(jid);
    if (accountId) {
    ...
    if (
      ...
      logger.warn(
        { jid, expectedAccountId: accountId },
        'Channel pairing target conflicts with an existing binding',
      );
      return { ok: false, reason: 'target_conflict' };
    }

    const pairingUserHome = getUserHomeGroup(claims.userId);
    if (!pairingUserHome) {
      logger.warn(
        { userId: claims.userId, expectedAccountId: accountId },
        'Cannot pair channel chat: owner workspace is missing',
      );
      return { ok: false, reason: 'workspace_missing' };
    }

    try {
      buildOnNewChat(claims.userId, pairingUserHome.folder)(jid, chatName);
      const group = registeredGroups[jid] ?? getRegisteredGroup(jid);
      if (!group) return { ok: false, reason: 'registration_failed' };
      ...
      if (!consumePairingCode(code)) {
        removeImGroupRecord(
          jid,
          'Rolled back channel pairing: pairing code was consumed concurrently',
        );
      ...
      logger.error(
        { err: error, jid, expectedAccountId: accountId },
        'Channel pairing registration failed',
      );
...
async function main(): Promise<void> {
  ...
  const recoveredHrAnalysisJobs = startHrAnalysisQueueWorker();
  if (recoveredHrAnalysisJobs > 0) {
    logger.info(
      { count: recoveredHrAnalysisJobs },
      'Recovered interrupted HR analysis jobs',
    );
  }

  try {
    const expiredHrJobs = scanExpiredHrJobs();
    if (expiredHrJobs > 0) {
      logger.info(
        { count: expiredHrJobs },
        'Closed expired HR jobs at startup',
      );
    }
  } catch (err) {
    logger.warn({ err }, 'HR job expiry scan failed at startup');
  }
  setInterval(
    () => {
      try {
        ...
        logger.warn({ err }, 'HR job expiry scan failed');
      }
    },
    60 * 60 * 1000,
  );
...
main().catch((err) => {
  logger.error({ err }, 'Failed to start iclaw');
  process.exit(1);
});

...
export function recordRunContextSnapshot(input: {
  ...
  const snapshot: RunContextSnapshot = {
    ...
    agentProfile: audit.agentProfile
      ...
      : null,
    prompt: {
      planHash: audit.iclawPrompt.planHash ?? null,
      totalBytes: audit.iclawPrompt.totalBytes,
      estimatedTokens: nullableNumber(audit.iclawPrompt.estimatedTokens),
      blocks: audit.iclawPrompt.files.map((block) => ({

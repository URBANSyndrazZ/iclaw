...
export function getAgentBuilderRuntimeRejection(input: {
  ...
  if (!input.sourceProfileIsDefault) {
    return 'Only the main iclaw may use Agent Builder';
  }
  return null;
}
...
export function resolveTrustedDirectOwnerUpgrade(
  ...
  // older iclaw version. Never convert such a historical anchor into
...
export class AgentBuilderTurnRegistry {
  ...
  private readonly recentlyCompleted = new Map<
    string,
    AgentBuilderTurnContext[][]
  >();
  ...
  clearCompleted(
    ...
    const completedBatches = [
      ...
      ...queued,
    ].filter((batch) => batch.some((turn) => completedKeys.has(key(turn))));
    const recognized = completedBatches.length > 0;
    if (!recognized) return;

    const recent = this.recentlyCompleted.get(folder) ?? [];
    this.recentlyCompleted.set(
      folder,
      [...recent, ...completedBatches].slice(-8),
    );
  ...
  delete(folder: string): void {
    this.active.delete(folder);
    this.queued.delete(folder);
    this.recentlyCompleted.delete(folder);
  }
  ...
  /**
   * First-wake acknowledgement is emitted while Assistant output is completing,
   * so it can legitimately arrive after clearCompleted() promotes the next
   * queued batch. Keep a short exact-turn history solely for this one-shot ACK;
   * every durable input is revalidated here, so stale or non-owner turns still
   * fail closed.
   */
  requireExactRecentlyCompletedOwnerHumanTurn(
    folder: string,
    runtimeTurnId: string | null | undefined,
    loadPersistedInput: (
      chatJid: string,
    ...
    const batches = [
      ...(this.active.has(folder) ? [this.active.get(folder)!] : []),
      ...(this.queued.get(folder) ?? []),
      ...(this.recentlyCompleted.get(folder) ?? []),
    ];
    const exact = batches.find(
      (batch) =>
        batch.length > 0 &&
    ...
    if (!exact) {
      throw new Error(
        'Owner Profile requires the exact recently completed conversation turn',
      );
    }
    const observed = exact.map((turn) => ({
      turn,
    ...
    for (const { turn, input } of observed) {
      if (
        ...
        throw new Error(
          'Owner Profile acknowledgement is unavailable for this turn',
        );
      }
    }
    const current = observed[observed.length - 1];

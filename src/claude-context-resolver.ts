 ...
 * layer. Skills stay out of this list because iclaw governs them with an
...
export interface ClaudeContextPlanArgs {
   ...
   * context. Managed iclaw skills are resolved independently below.
   */
 ...
 * order. The caller applies iclaw's required env and effective MCP policy
...
export function loadHostClaudeSettings(
  ...
  for (const settingsFile of plan.settingsSourceFiles) {
    ...
    try {
      ...
      // directly. iclaw skips the invalid layer to keep startup usable.
    }
  }
  return merged;
}
...
export function buildClaudeContextPlan(
  ...
  const audit: ClaudeContextAudit = {
    ...
    iclawPrompt: { totalBytes: 0, files: [] },
    warnings,
  };
...
export function syncHostClaudeContext(
  ...
  // These paths are iclaw-owned projections of the selected native user

...
async function isBinaryAvailable(binary: string): Promise<boolean> {
  if (binary === 'node') {
    return resolveHostNodeBinary(process.env) !== 'node'
      ? true
      : (await locateBinaryOnPath(binary)) !== null;
  }
  return (await locateBinaryOnPath(binary)) !== null;
}

/** Resolve the actual path of a host capability binary. */
async function resolveBinaryPath(binary: string): Promise<string | null> {
  if (binary === 'node') {
    const resolved = resolveHostNodeBinary(process.env);
    return resolved !== 'node' ? resolved : locateBinaryOnPath(binary);
  }
  return locateBinaryOnPath(binary);
}

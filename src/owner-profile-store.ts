...
import {
  iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY,
  WorkspaceMemoryStoreError,
  enforceiclawOwnerAddressCanonicalInvariant,
  geticlawOwnerAddressMemoryItem,
  mutateiclawOwnerAddressMemory,
  reconcileiclawOwnerAddressCanonicalKey,
...
export const iclaw_OWNER_INTRODUCTION_FLOW_KEY =
  'iclaw.owner-introduction';
...
export type iclawOwnerIntroductionState =
  | 'pending'
  | 'claimed'
  | 'completed'
  | 'skipped';
...
export interface iclawOwnerProfileProjection {
  ...
  onboarding: {
    flowKey: typeof iclaw_OWNER_INTRODUCTION_FLOW_KEY;
    state: iclawOwnerIntroductionState;
    revision: number;
...
function requireHomeWorkspace(workspaceJid: string): void {
  ...
  if (!row) {
    throw new OwnerProfileStoreError(
      'not_home_workspace',
      'iclaw Owner Profile is available only in the owner Home Workspace',
    );
  }
}
...
function onboardingRow(workspaceJid: string): OnboardingRow | null {
  return (
    (requireDatabase()
      ...
      .get(workspaceJid, iclaw_OWNER_INTRODUCTION_FLOW_KEY) as
      | OnboardingRow
      | undefined) ?? null
  );
}
...
function ensureOnboardingRow(workspaceJid: string, at: string): OnboardingRow {
  ...
  ).run(workspaceJid, iclaw_OWNER_INTRODUCTION_FLOW_KEY, at, at);
  ...
  db.exec(`
    ...
    BEGIN
      INSERT OR IGNORE INTO workspace_onboarding_states (
        ...
        NEW.jid, 'iclaw.owner-introduction', 'pending', 0, NULL, 0,
    ...
    SELECT
      jid, 'iclaw.owner-introduction', 'pending', 0, NULL, 0,
...
export function geticlawOwnerProfileProjection(
  workspaceJid: string,
): iclawOwnerProfileProjection {
  requireHomeWorkspace(workspaceJid);
  const { item } = geticlawOwnerAddressMemoryItem(workspaceJid);
  ...
  return {
    ...
    onboarding: {
      flowKey: iclaw_OWNER_INTRODUCTION_FLOW_KEY,
...
function completeIntroduction(workspaceJid: string, at: string): void {
  ...
  requireDatabase()
    ...
    .run(at, at, workspaceJid, iclaw_OWNER_INTRODUCTION_FLOW_KEY);
}

export function seticlawOwnerPreferredAddress(input: {
  workspaceJid: string;
  ...
  try {
    return db.transaction(() => {
      const mutation = mutateiclawOwnerAddressMemory({
      ...
      return {
        projection: geticlawOwnerProfileProjection(input.workspaceJid),
...
export function cleariclawOwnerPreferredAddress(input: {
  workspaceJid: string;
  expectedRevision: number;
  idempotencyKey?: string | null;
  context: WorkspaceMemoryMutationContext;
...
export function claimiclawOwnerIntroduction(input: {
  workspaceJid: string;
  leaseOwner: string;
  leaseMs?: number;
  now?: string;
  ...
  const db = requireDatabase();
  return db.transaction(() => {
    const profile = geticlawOwnerAddressMemoryItem(input.workspaceJid);
    ...
    if (leaseIsCurrent && row.lease_owner === leaseOwner) {
      db.prepare(
        ...
        at,
        input.workspaceJid,
        iclaw_OWNER_INTRODUCTION_FLOW_KEY,
        leaseOwner,
      );
      return {
        claimed: true,
 ...
 */
export function acknowledgeiclawOwnerIntroduction(input: {
  workspaceJid: string;
  leaseOwner: string;
  leaseToken: number;
  now?: string;
 ...
 */
export function releaseiclawOwnerIntroductionLease(
  leaseOwner: string,
  now: string = nowIso(),
): number {
...
export function skipiclawOwnerIntroduction(input: {
  workspaceJid: string;
  ...
  return db.transaction(() => {
    ...
    if (
      ...
      throw new OwnerProfileStoreError(
        'revision_conflict',
        'iclaw onboarding revision conflict',
    ...
    db.prepare(
      `UPDATE workspace_onboarding_states
       ...
       WHERE workspace_jid = ? AND flow_key = ?`,
    ).run(at, at, input.workspaceJid, iclaw_OWNER_INTRODUCTION_FLOW_KEY);
    return {
      projection: geticlawOwnerProfileProjection(input.workspaceJid),
      changed: true,
    };
...
export function reconcileLegacyOwnerProfileMemory(): {
  ...
  const db = requireDatabase();
  return db.transaction(() => {
    const reconciled = reconcileiclawOwnerAddressCanonicalKey();
    const homes = db
      .prepare(`SELECT jid FROM workspaces WHERE is_home = 1`)
      .all() as Array<{ jid: string }>;
    for (const home of homes) {
      const at = nowIso();
      const { item } = geticlawOwnerAddressMemoryItem(home.jid);
      ...
      if (item.status === 'active' && legacyAddressMeansSkip(item.content)) {
        mutateiclawOwnerAddressMemory({
          ...
          context: {
            ...
            sourceId: iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY,
            observedAt: at,
          },
        });
        db.prepare(
          `UPDATE workspace_onboarding_states
           ...
           WHERE workspace_jid = ? AND flow_key = ?`,
        ).run(at, at, home.jid, iclaw_OWNER_INTRODUCTION_FLOW_KEY);
      } else {

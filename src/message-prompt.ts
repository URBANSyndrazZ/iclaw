...
function documentAttachmentHints(message: NewMessage): string[] {
    ...
    return parsed
      .filter(
        (item): item is Record<string, unknown> =>
          Boolean(item) &&
          typeof item === 'object' &&
          (item as Record<string, unknown>).type === 'document',
      )
      .map((item) => {
        const attachmentId = String(item.attachmentId ?? '');
        const name = String(item.name ?? '');
        return attachmentId && name
          ? `[简历附件 name="${escapeXml(name)}" attachment_id="${escapeXml(attachmentId)}"]`
          : '';
      })
      .filter(Boolean);
  } catch {
...
export function formatMessages(
  ...
  const lines = messages.map((message) => {
    ...
    const documentHints = documentAttachmentHints(message);
    const documentBlock = documentHints.length
      ? `${documentHints.join('\n')}\n`
      : '';
    ...
    const formatted =
      ...
      `${referenceBlock}${documentBlock}${escapeXml(message.content)}</message>`;

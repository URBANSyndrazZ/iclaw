...
import { iclawOwnerProfileMutationSchema } from '../schemas.js';
import {
...
workspaceRoutes.get('/:jid/owner-profile', authMiddleware, (c) => {
  ...
  try {
    return c.json({
      profile: geticlawOwnerProfileProjection(access.workspace.jid),
    });
  } catch (error) {
...
workspaceRoutes.patch('/:jid/owner-profile', authMiddleware, async (c) => {
  ...
  const parsed = iclawOwnerProfileMutationSchema.safeParse(body);
  if (!parsed.success) {
    return c.json(

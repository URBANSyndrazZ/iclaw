...
/** One durable Home Workspace fact completes iclaw's first-wake ritual. */
export const iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY =
  'iclaw.owner.preferred_address';
const iclaw_OWNER_PROFILE_MEMORY_NAMESPACE = 'iclaw.owner-profile';
...
function isReservedCanonicalKey(value: string | null | undefined): boolean {
  return (
    normalizeNullable(value) === iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY
  );
}
...
function rejectReservedCanonicalKey(value: string | null | undefined): void {
  ...
  throw new WorkspaceMemoryStoreError(
    'reserved_canonical_key',
    'This canonical key is owned by the iclaw Owner Profile service',
  );
}
...
function isReservedMemoryItem(item: WorkspaceMemoryItem): boolean {
  ...
  return Boolean(
    requireDatabase()
      ...
      .get(item.id, item.id, iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY),
  );
}
...
function markPlatformMemoryItem(itemId: string, at: string): void {
  requireDatabase()
    ...
    .run(itemId, iclaw_OWNER_PROFILE_MEMORY_NAMESPACE, at);
}
...
function rejectReservedMemoryItem(item: WorkspaceMemoryItem): void {
  ...
  throw new WorkspaceMemoryStoreError(
    'reserved_canonical_key',
    'This memory item is owned by the iclaw Owner Profile service',
  );
}
...
export function createWorkspaceMemorySchema(db: SqliteDatabase): void {
  db.exec(`
    ...
    SELECT
      id, 'iclaw.owner-profile', created_at
    FROM workspace_memory_items
    WHERE canonical_key = 'iclaw.owner.preferred_address';

    INSERT OR IGNORE INTO workspace_memory_platform_items (
      item_id, namespace, created_at
    )
    SELECT
      item_id, 'iclaw.owner-profile', MIN(created_at)
    FROM workspace_memory_versions
    WHERE canonical_key = 'iclaw.owner.preferred_address'
    GROUP BY item_id;
  `);
 ...
 */
export function reconcileiclawOwnerAddressCanonicalKey(): {
  workspaces: number;
  retiredItems: number;
} {
  const db = requireDatabase();
  return db.transaction(() => {
    const duplicateStores = db
      ...
      .all(iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY) as Array<{
      store_id: string;
    }>;
    let retiredItems = 0;
    for (const duplicate of duplicateStores) {
      ...
      for (const loser of rows.slice(1)) {
        ...
        const context: WorkspaceMemoryMutationContext = {
          ...
          sourceId: 'iclaw.owner.preferred_address',
          observedAt: at,
        };
 ...
 */
export function enforceiclawOwnerAddressCanonicalInvariant(): void {
  requireDatabase()
    .prepare(
    ...
    .run(
      iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY,
      iclaw_OWNER_PROFILE_MEMORY_NAMESPACE,
    );
 ...
 *
 * iclaw may temporarily disable SQLite foreign-key enforcement when a
 ...
 */
export function geticlawOwnerAddressMemoryItem(workspaceJid: string): {
  store: WorkspaceMemoryStore;
  ...
  const row = requireDatabase()
    ...
    .get(store.id, iclaw_OWNER_PREFERRED_ADDRESS_CANONICAL_KEY) as
    | MemoryItemRow
    | undefined;
 ...
 */
export function mutateiclawOwnerAddressMemory(input: {
  workspaceJid: string;
  ...
  return db.transaction(() => {
    const { store: initialStore, item: current } =
      geticlawOwnerAddressMemoryItem(input.workspaceJid);
    const idempotencyKey = normalizeNullable(input.idempotencyKey);
    ...
    if (!current) {
      if (input.clear) {
        throw new WorkspaceMemoryStoreError(
          'revision_conflict',
          'iclaw Owner Profile revision conflict',

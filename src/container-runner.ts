...
import { ensureClaudeJsonLink } from './claude-json-link.js';
import {
...
import { releaseiclawOwnerIntroductionLease } from './owner-profile-store.js';
import {
  deleteSession,
  getUserById,
...
export interface ContainerInput {
  ...
  agentProfile?: {
    ...
    name: string;
    version: number;
    /** Host-authoritative marker for the built-in iclaw AgentProfile. */
    isDefault: boolean;
    identityHash: string;
  ...
  /**
   * True only for an interactive turn in the built-in iclaw Home
   * Workspace before the owner-preferred-address memory has been established.
   */
  iclawBootstrapPending?: boolean;
  /**
   * Host-authoritative initial Owner Profile projection. Present only for an
   * actual-owner interactive turn of built-in iclaw in Home. The runner
  ...
  iclawOwnerProfileEnabled?: boolean;
  /** Host-derived capability flag for main-iclaw interactive sessions. */
  agentBuilderEnabled?: boolean;
  /** Host-authoritative HR Agent capability switch for this workspace. */
...
export interface ContainerOutput {
  ...
  /** Set when the runner never started and retrying cannot fix the failure. */
...
export function replaceHostMcpServersEnv(
  ...
  delete env['iclaw_USER_MCP_SERVERS_JSON'];
  if (Object.keys(servers).length > 0) {
    env['iclaw_USER_MCP_SERVERS_JSON'] = JSON.stringify(servers);
  }
}
...
export function applyFallbackModelToEnvLines(
  ...
  for (let i = envLines.length - 1; i >= 0; i--) {
    if (envLines[i].startsWith('iclaw_FALLBACK_MODEL=')) {
      envLines.splice(i, 1);
    }
  }
  const fallbackModel = configuredFallbackModel?.trim();
  if (!fallbackModel) return;
  envLines.push(`iclaw_FALLBACK_MODEL=${fallbackModel}`);
}
...
export function buildVolumeMounts(
  ...
  ensureSettingsJson(
    ...
    resolveRuntimeMcpServers(group, agentProfile),
    {
      // The session settings file is iclaw-owned. Always replace this map
  ...
  // authorization, forcing re-auth every IDLE_TIMEOUT (#477). iclaw never
  ...
  for (let index = envLines.length - 1; index >= 0; index -= 1) {
    if (
      ...
      envLines[index]?.startsWith('iclaw_AGENT_TOOL_POLICY=') ||
      envLines[index]?.startsWith('iclaw_AGENT_DISALLOWED_TOOLS=') ||
      envLines[index]?.startsWith('iclaw_AGENT_MCP_POLICY=')
    ) {
  ...
  if (mcpPolicyMode !== 'inherit') {
    envLines.push(`iclaw_AGENT_MCP_POLICY=${mcpPolicyMode}`);
  }
  ...
  // build (e.g. identity.iclaw.md) is missing inside the container while
...
export function detectContainerHostIdentity(
  ...
  // while iclaw is running; reusing an earlier direct result could bypass
...
export function buildContainerArgs(
  ...
  args.push('-e', `TZ=${tz}`);
  args.push('-e', `iclaw_HOST_IDENTITY_MODE=${hostIdentity.mode}`);
  if (hostIdentity.mode === 'direct') {
    if (isPositiveUnixId(hostIdentity.uid)) {
      args.push('-e', `iclaw_HOST_UID=${hostIdentity.uid}`);
    }
    if (isPositiveUnixId(hostIdentity.gid)) {
      args.push('-e', `iclaw_HOST_GID=${hostIdentity.gid}`);
    }
  }
...
export async function runContainerAgent(
  ...
  try {
    ...
    const agentSuffix = sessionAgentId
      ...
      : '';
    const containerName = `iclaw-${safeName}${agentSuffix}-${Date.now()}`;
    ...
    const result = await new Promise<ContainerOutput>((resolve) => {
      ...
      container.on('error', (err) => {
        ...
        const startupFailureTerminal =
          (err as NodeJS.ErrnoException).code === 'ENOENT';
        const error = startupFailureTerminal
          ? 'Docker command not found. Install Docker or set ICLAW_SINGLE_HOST_MODE=true for single-host development.'
          : `Container spawn error: ${err.message}`;
        resolve({
          status: 'error',
          result: startupFailureTerminal ? error : null,
          error,
          startupFailureTerminal,
        });
      });
    });
    ...
    try {
      releaseiclawOwnerIntroductionLease(workspaceMemoryRunnerInstanceId);
    } catch (err) {
      logger.warn(
...
export async function runHostAgent(
  ...
  try {
    ensureClaudeJsonLink(localJson, ensureHostClaudeJson());
  } catch (err) {
    logger.warn(
      { err, localPath: localJson, targetPath: getHostClaudeJsonPath() },
      'Failed to link .claude.json, deviceId may differ',
    );
  }
  ...
  delete hostEnv['iclaw_USER_MCP_SERVERS_JSON'];
  // Provider selection is authoritative. Do not let API settings inherited
  // from the iclaw parent process leak into a host-mode child after the
  ...
  // processes. When iclaw is started by a background process manager
  ...
  try {
    ...
    if (fallbackModel) {
      hostEnv['iclaw_FALLBACK_MODEL'] = fallbackModel;
    } else {
      delete hostEnv['iclaw_FALLBACK_MODEL'];
    }
    ...
    delete hostEnv['iclaw_AGENT_TOOL_POLICY'];
    delete hostEnv['iclaw_AGENT_DISALLOWED_TOOLS'];
    delete hostEnv['iclaw_AGENT_MCP_POLICY'];
    if (hostMcpPolicyMode !== 'inherit') {
      hostEnv['iclaw_AGENT_MCP_POLICY'] = hostMcpPolicyMode;
    }

    // 路径映射
    hostEnv['iclaw_WORKSPACE_GROUP'] = groupDir;
    hostEnv['iclaw_WORKSPACE_IPC'] = groupIpcDir;

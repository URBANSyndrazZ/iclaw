...
const RESERVED_CLAUDE_ENV_KEYS = new Set([
  ...
  'ANTHROPIC_MODEL',
  'iclaw_CLAUDE_ENDPOINT_KIND',
  'iclaw_FALLBACK_MODEL',
]);

export const CLAUDE_ENDPOINT_KIND_ENV = 'iclaw_CLAUDE_ENDPOINT_KIND';
...
/**
 * Remove provider-owned values inherited from the iclaw parent process.
...
const DANGEROUS_ENV_VARS = new Set([
  ...
  'TMP',
  // iclaw 内部路径映射
  'iclaw_WORKSPACE_GROUP',
  'iclaw_WORKSPACE_GLOBAL',
  'iclaw_WORKSPACE_MEMORY',
  'iclaw_WORKSPACE_IPC',
  'CLAUDE_CONFIG_DIR',
]);
...
function isDangerousEnvKey(key: string): boolean {
  return (
    ...
    key.startsWith('iclaw_SESSION_') ||
    key.startsWith('iclaw_INTERNAL_') ||
    key === 'iclaw_HOST_IDENTITY_MODE' ||
    key === 'iclaw_HOST_UID' ||
    key === 'iclaw_HOST_GID' ||
    key === 'iclaw_PASSWD_FILE' ||
    key === 'iclaw_RECONCILE_SESSION_PERMISSIONS' ||
    key === 'iclaw_MOUNT_PREPARE_MODE' ||
    key === 'iclaw_RUNTIME_USER'
  );
}
...
function fromStoredProfile(
  ...
  return {
    ...
    anthropicModel: normalizeModel(
      stored.anthropicModel ?? (stored as any).iclawModel ?? '',
    ),
...
export function getContainerEnvConfig(folder: string): ContainerEnvConfig {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      ) as ContainerEnvConfig & { iclawModel?: string };
      // Backward compat: migrate old field name
      if (
        stored.anthropicModel === undefined &&
        stored.iclawModel !== undefined
      ) {
        stored.anthropicModel = stored.iclawModel;
        delete stored.iclawModel;
      }
      return stored;
    }
  } catch (err) {

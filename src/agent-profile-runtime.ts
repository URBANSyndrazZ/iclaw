...
/**
 * The built-in iclaw identity is code-owned rather than persisted in an
 ...
 */
export const iclaw_PLATFORM_IDENTITY_VERSION = 1;

export function bindiclawPlatformIdentityHash(
  identityHash: string,
): string {
  return createHash('sha256')
    .update(
      `iclaw-platform-identity:v${iclaw_PLATFORM_IDENTITY_VERSION}\0${identityHash}`,
      'utf8',
    )
    .digest('hex');
}
...
export function resolveEffectiveAgentProfile(
  ...
  return {
    ...
    identity_hash: profile.is_default
      ? bindiclawPlatformIdentityHash(effectiveIdentityHash)

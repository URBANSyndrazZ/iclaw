...
app.post('/api/messages', authMiddleware, async (c) => {
  ...
  if (isHostExecutionGroup(group) && !canRunHostAgent(authUser)) {
    return c.json(
...
function setupWebSocket(server: any): WebSocketServer {
  ...
  server.on('upgrade', (request: any, socket: any, head: any) => {
    ...
    request.__iclawSessionId = token;
  ...
  wss.on('connection', (ws, request: any) => {
    const sessionId = request?.__iclawSessionId as string | undefined;
    ...
    if (connSession && deps) {
      ...
      for (const g of queueStatus.groups) {
        ...
        if (
          allowed === null ||
          !allowed.has(userId) ||
          runtimeBroadcastAdminOnly(baseJid)
        ) {
          continue;
        }
...
/**
 * Host execution is an authorization boundary, but single-host mode explicitly
 * admits the active workspace owner as an Agent runtime principal. Keep true
 * admin-only broadcasts for administrator operations while allowing owner-scoped
 * runtime events through only when that mode is enabled.
 */
function runtimeBroadcastAdminOnly(chatJid: string): boolean {
  return (
    isHostGroupJid(stripRuntimeJidSuffix(chatJid)) && !isSingleHostModeEnabled()
  );
}
...
export function broadcastNewMessage(
  ...
  safeBroadcast(wsMsg, runtimeBroadcastAdminOnly(baseChatJid), allowedUserIds);
}
...
export function broadcastFollowUpUpdate(
  ...
  safeBroadcast(msg, runtimeBroadcastAdminOnly(baseChatJid), allowedUserIds);
}
...
export function broadcastTyping(chatJid: string, isTyping: boolean): void {
  ...
  safeBroadcast(
    ...
    runtimeBroadcastAdminOnly(chatJid),
    allowedUserIds,
  );
}
...
export function broadcastStreamEvent(
  ...
  safeBroadcast(msg, runtimeBroadcastAdminOnly(chatJid), allowedUserIds);
...
export function broadcastRunnerState(
  ...
  safeBroadcast(msg, runtimeBroadcastAdminOnly(baseJid), allowedUserIds);

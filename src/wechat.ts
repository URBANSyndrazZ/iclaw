...
import {
  isPairingInfrastructureFailure,
...
export interface WeChatConnectionConfig {
  ...
  /** Bypass iclaw's HTTP(S) proxy. System TUN/VPN routing still applies. */
  bypassProxy?: boolean;
  logContext?: {
    accountId?: string;
    userId?: string;
...
/** Map an internal pairing outcome to the reply the WeChat user should see. */
export function weChatPairReply(result: ChannelPairAttemptResult): string {
  const succeeded =
    result === true || (typeof result === 'object' && result.ok);
  if (succeeded) return '配对成功，此微信会话已连接。';
  return isPairingInfrastructureFailure(result)
    ? '配对失败，请稍后重试。'
...
export function weChatConnectionErrorMessage(
  ...
  switch (code) {
    case 'connect_timeout':
      return '连接微信服务超时，iclaw 正在自动重试';
    case 'request_timeout':
      return '微信长轮询暂时无响应，iclaw 正在自动重试';
    case 'connection_reset':
      return '微信连接在 TLS 建立前被中断，iclaw 正在自动重试';
    case 'tls_error':
      return '微信服务 TLS 连接失败，iclaw 正在自动重试';
    case 'api_error':
      return '微信服务返回异常，iclaw 正在自动重试';
    case 'network_error':
      return '暂时无法访问微信服务，iclaw 正在自动重试';
    default:
      return '微信连接暂时异常，iclaw 正在自动重试';
  }
}
...
export function createWeChatConnection(
  ...
  function baseInfo(): Record<string, string> {
    return {
      ...
      bot_agent: `iclaw/${CHANNEL_VERSION}`,
    };
  }
  ...
  async function processMessage(
    ...
    try {
      ...
      const pairMatch = parseChannelPairingCode(content);
      if (pairMatch && opts.onPairAttempt) {
        try {
          const pairResult = await opts.onPairAttempt(jid, chatName, pairMatch);
          const retryPairingService =
            isPairingInfrastructureFailure(pairResult);
          const failedResult =
            typeof pairResult === 'object' && !pairResult.ok
              ? pairResult
              : undefined;
          const failureReason =
            pairResult === false ? 'unspecified' : failedResult?.reason;
          if (!retryPairingService && (pairResult === false || failedResult)) {
            logger.warn(
              {
                jid,
                reason: failureReason,
              },
              'WeChat pairing code rejected',
            );
          }

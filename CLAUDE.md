...
- HR 业务架构、API 和验收边界：`docs/HR-AGENT-WORKBENCH.md`
...
- HR 工作台：`docs/HR-AGENT-WORKBENCH.md`
...
iclaw 是由 Pi Agent Runtime 驱动的 HR Agent 工作台。产品主入口是 `/hr`，面向
技术招聘的 JD、简历、候选人、面试反馈、评分规则和飞书同步闭环。

平台保留 Pi Agent Runtime、Workspace、Memory、Skills、MCP、Plugins、任务和
多渠道触达作为底层能力，但不把它们作为主要产品定位。任何工程改动都不应把
这些支撑层重新提升为首页卖点。
...
| `src/hr/`                          | HR 领域服务：简历导入、AI 分析、飞书同步与持久化       |
...
| `/users`                                                              | 用户、邀请和审计管理                      |
| `/hr`、`/hr/jobs`、`/hr/candidates`、`/hr/questions`、`/hr/analytics` | HR 总览、岗位库、候选人中心、问题库与诊断 |
| `/hr/jobs/:jobId`                                                     | 岗位详情、导入、评分规则审核与诊断        |
| `/hr/candidates/:candidateId`                                         | 候选人画像、评分、面问题与反馈            |
...
- iclaw MCP 工具由 `container/agent-runner/src/mcp-tools.ts` 注册。
...
HR 领域表由 `src/hr/store.ts` 在同一数据库连接上创建，包括岗位、候选人、简历、匹配、
面问题、面试轮次、分析任务、飞书配置/同步和审计。不要直接从前端暴露简历路径或完整提取文本。
...
| `CONTAINER_IMAGE`           | `URBANSyndrazZ/iclaw-agent:latest` | GitHub Actions 发布的 Runner 镜像 |

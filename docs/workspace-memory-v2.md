...
Workspace Memory v2 是 iclaw 的工作区长期知识层。它保存经过提炼、未来
Session 仍可复用的内容，并以 Workspace 作为唯一产品入口和权限边界。

## 产品边界

iclaw 的相关数据分成三个彼此独立的层次：

| 层次                   | 用途                                     | 生命周期                         |
| ---------------------- | ---------------------------------------- | -------------------------------- |
| 平台/AgentProfile 身份 | Agent 是谁、平台规则和能力策略           | 配置或代码版本管理，不可被忘记   |
| Home Owner Profile     | 内置 iclaw 如何称呼实际 owner        | Home 跨 Session，专用 API 管理   |
...
- 内置 iclaw 的平台身份和安全约束不是 Memory；自定义 Agent 的身份由各自
  AgentProfile 管理，也不会继承 iclaw 的内置身份。
...
- 每个用户的 Home Workspace 永久绑定内置 iclaw，不能删除或迁移给自定义
  Agent。
...
Workspace Memory v2 的 canonical store 位于 iclaw SQLite 主数据库。每个
...
Owner Profile 的边界更严格：只允许 Home 内置 iclaw 的 Main/普通 Runtime

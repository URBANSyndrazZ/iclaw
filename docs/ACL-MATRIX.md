...
本文按底层平台权限边界描述。产品主入口是 HR Agent 工作台；HR 数据的
owner 隔离、简历下载、Agent 工具访问和飞书配置边界见
[`HR-AGENT-WORKBENCH.md`](HR-AGENT-WORKBENCH.md)。
...
- `ICLAW_SINGLE_HOST_MODE=true` 是本地开发/演示例外，只允许 active 用户运行 Home
  Agent runtime；脚本、额外 host 挂载和其他管理员级 host 权限仍要求 admin。
- 非 owner 的用户即使拥有系统级 Permission，也不能读取或修改其他用户工作区中的
  Secret、环境变量和运行能力。
- HR 工作台沿用同一套认证、owner 和审计边界；HR Agent 工具不能跨 owner 查询、
  修改或同步候选人数据。
...
| iclaw Owner Profile                                 | Home actual owner + runtime exact-turn gate   |
...
iclaw。路由在进入 Runtime quiesce 前拒绝迁移，数据库绑定函数再次校验；启动
...
### 4.6 iclaw Owner Profile

`GET|PATCH /api/workspaces/:jid/owner-profile` 只接受当前登录用户自己的 Home
Workspace，且目标必须绑定内置默认 iclaw。admin 不具有跨 owner bypass。

Runtime 的权限更严格：

| 上下文                                       | 投影                | 修改                  |
| -------------------------------------------- | ------------------- | --------------------- |
| Home 内置 iclaw + actual owner turn          | 允许                | 当前 active turn 允许 |
| Home 内置 iclaw + 非 owner 群成员 turn       | 拒绝                | 拒绝                  |
| Home 内置 iclaw 普通 Runtime Session         | actual owner 时允许 | 当前 active turn 允许 |

# iclaw Claude Code 插件自动化导入与运行设计方案
...
iclaw 通过 `@anthropic-ai/claude-agent-sdk` 启动 Claude Code Agent，并使用 SDK
...
3. 带 `disable-model-invocation: true` 的 slash command 在 SDK 模式下不会被模型主动调用；这类命令在 Claude Code REPL 中属于“用户手动输入后由 CLI 展开”的命令，需要 iclaw 在消息进入 Agent 前补齐展开逻辑。
...
1. **原生优先**
   ...
   Plugin 的资源加载继续走 Claude Code SDK/CLI 原生机制。iclaw 只负责导入、启用、路径转换和必要的 slash command 展开。
...
3. **不把 Markdown command 当脚本接口**
   ...
   Claude Code command 是 Markdown prompt，加 frontmatter 和动态上下文。iclaw 不应把 fenced bash 块当作本地脚本直接执行。只有 Claude Code 明确定义的 inline `!` bash context 可以作为 command expansion 的一部分处理。

4. **手动命令才展开 `disable-model-invocation`**

   `disable-model-invocation: true` 的含义是禁止模型通过 SlashCommand tool 自动触发，但用户手动输入 `/xxx` 仍应可执行。iclaw 的展开逻辑只针对用户真实消息，不提供给模型自动调用。
...
```json
{
  "sources": [
    ...
    {
      "type": "directory",
      "path": "/opt/iclaw/plugin-marketplaces"
    }
  ]
}
```
...
1. IM 端仍先走 `handleCommand()` 处理 iclaw 内置命令。
...
5. 命中且 DMI=true：
   - 说明这是“用户手动命令”，允许 iclaw 展开为 Agent prompt。
...
对于这类内容，iclaw 展开后交给 Agent，由 Claude Code SDK 中的工具机制执行。
...
如果后续需要 iclaw 自管后台任务，应另行设计通用 job system，而不是写 codex 特例。
...
1. **无法 100% 复刻 Claude Code REPL**
   ...
   Claude Code REPL 内部可能有未公开行为。方案尽量复用 SDK/CLI 原生 plugin 加载，iclaw 只补 command expansion。

2. **复杂交互 command 仍依赖 Agent**

   AskUserQuestion、Task、background Bash、Skill 等语义不在 iclaw 本地执行。它们通过展开后的 prompt 交给 Agent 处理。
...
完成后，iclaw 的 Claude Code plugin 能力将形成完整闭环：

1. Admin 安装或更新 Claude Code plugin。
2. iclaw 自动扫描并导入 catalog。
3. 用户从 catalog 启用插件。
4. iclaw materialize 用户 runtime，并在 Agent 启动时通过 SDK `options.plugins` 注入。
5. 普通 plugin 资源由 Claude Code 原生加载。
6. 用户手动输入的 DMI slash command 由 iclaw 展开为 Agent prompt，补齐 SDK 模式缺口。

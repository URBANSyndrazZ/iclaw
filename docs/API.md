...
- 除明确标记 Public 的接口外，均需要有效的 iclaw Cookie Session。
...
HR 工作台是产品主入口。`/api/hr/*` 是业务 API；工作区、Agent、渠道、任务、
能力库和计费接口是支撑 HR Agent 运行的高级 API，仅在需要扩展平台能力时使用。

## 路由模块

| 前缀                               | 实现                             | 用途                          |
| ---------------------------------- | -------------------------------- | ----------------------------- |
| `/api/hr`                          | `src/routes/hr.ts`               | HR 工作台、简历与飞书同步     |
...
所有接口都需要登录，并且只访问当前用户的 HR 数据。

- `GET|POST /api/hr/jobs`
- `GET /api/hr/jobs?with_stats=true`，返回每个岗位的候选人阶段统计
- `GET /api/hr/overview`，返回工作台 KPI、待处理事项、活跃岗位和最近候选人
- `GET /api/hr/overview/export.csv`，下载工作台汇总 CSV；不含候选人姓名和联系方式
- `GET|PUT /api/hr/jobs/:jobId`
- `GET /api/hr/jobs/:jobId/rules`
- `GET /api/hr/candidates`，支持 `jobId`、`stage`、`talentPool`、`search`、`limit`、`offset`
- `GET /api/hr/questions`，聚合候选人的 HR 面问题
- `GET /api/hr/analytics`，返回阶段漏斗、岗位诊断、JD 缺口、反馈偏差、拒绝原因和 AI 任务状态
- `POST /api/hr/candidates/import`，`multipart/form-data`：`job_id`、`file`、可选 `full_name`
- `POST /api/hr/chat-resume-uploads`，`multipart/form-data`：`file`；返回聊天简历附件
  `attachment_id`，供 `hr_analyze_resume_preview` 临时分析使用，文件按 owner 隔离保存
  并在分析后或 24 小时后清理
- `POST /api/hr/candidates/import/link`，传入必填 HTTPS `source_url` 后由 iclaw 服务端下载简历
- `GET /api/hr/candidates?cleanup_due=true`，返回保留期已到的已关闭候选人
- `GET|PATCH /api/hr/candidates/:candidateId`
  - PATCH 支持 `full_name`、`stage`、`talent_pool`、`summary` 和其他候选人资料字段
- `POST /api/hr/candidates/:candidateId/hard-delete`，`confirm_full_name` 必须匹配后执行彻底删除
- `POST /api/hr/candidates/:candidateId/reanalyze`，排队后台执行简历解析、JD 匹配和 HR 面问题生成
- `POST /api/hr/candidates/:candidateId/feedback`
- `GET /api/hr/resumes/:resumeId/download`
- `GET /api/hr/resumes/:resumeId/view`，PDF/TXT/MD 安全内联预览
- `DELETE /api/hr/resumes/:resumeId`，只删除该份简历文件和记录，并尝试同步清空飞书简历链接
- `POST /api/hr/candidates/:candidateId/resumes/:resumeId/replace`，`multipart/form-data`：`file`；替换成功后删除旧简历并重置 AI 分析状态
- `POST /api/hr/jobs/:jobId/market-analysis`
- `GET|PUT /api/hr/feishu/config`
- `GET /api/hr/feishu/status`，返回配置和最近同步状态
- `POST /api/hr/feishu/test`，校验 Feishu 凭证、Bitable 表和字段映射
- `POST /api/hr/candidates/:candidateId/feishu/sync`

简历下载返回文件流；预览仅支持 PDF 与 TXT/MD，MD/TXT 按纯文本返回。列表和详情 API
不返回简历保存路径或完整简历文本。
HTTPS 链接导入、AI schema、飞书字段映射和安全边界见
[HR Agent Workbench](HR-AGENT-WORKBENCH.md)。
...
Home Workspace 固定归属当前用户的内置 iclaw。尝试通过
`PATCH /api/groups/:jid/agent-profile` 将 Home 迁移到自定义 Agent 时返回：

```json
{
  "error": "Home Workspace 始终属于内置 iclaw，不能迁移到自定义智能体",
...
`host_path` 是 iclaw/Docker 守护进程所在服务器上的绝对目录，不是浏览器
...
显式选择它，或通过上面的非 Home Workspace 迁移接口建立归属。内置 iclaw 的
...
只允许该用户不可删除的 Home Workspace、内置默认 iclaw AgentProfile 和实际
...
通用 Memory API：`iclaw.owner.preferred_address` 不能通过通用接口创建、更新或

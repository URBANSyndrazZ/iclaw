...
| 真实 Provider 冒烟 | 当前 iclaw 已配置 Provider                             | 固定标记问答                                 | 模型完成请求且逐字返回固定标记                  |
...
- 期望：子请求包含 child marker、iclaw delegated-task contract 与 Skill 内容；不包含 main marker；子 Agent 不获得 Task 工具。

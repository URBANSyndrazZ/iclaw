...
const READY_FILE = '/run/iclaw-session-watcher.ready';
const FAILED_FILE = '/run/iclaw-session-watcher.failed';
...
const generatedRoots = new Map([
  ...
  [
    'chromium',
    {
      path: '/tmp/iclaw-chromium-profile',
      policy: 'managed',
      normalization: 'node-owned',
    },
  ],
...
function normalizeDescriptor(fd, root, required = false) {
  ...
  if (
    ...
    process.stderr.write(`iclaw: skipping ${description}\n`);
...
function stop() {
  ...
  try {
    ...
    process.stderr.write(
      `iclaw permission watcher final scan failed: ${error}\n`,
    );
    process.exit(1);
  }
}

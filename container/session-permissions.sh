...
iclaw_INTERNAL_IDENTITY_MODE=unconfigured
iclaw_INTERNAL_RUNTIME_UID=
iclaw_INTERNAL_RUNTIME_GID=
iclaw_INTERNAL_WATCHER_PID=

iclaw_permission_error() {
  printf 'iclaw: %s\n' "$1" >&2
}

iclaw_permission_fatal() {
  iclaw_permission_error "$1"
  return 1
}

iclaw_valid_nonroot_id() {
  local value="$1"
  case "$value" in
    '' | *[!0-9]*) return 1 ;;
  esac
...
iclaw_write_runtime_ids() {
  ...
  temporary=$(mktemp /etc/iclaw-passwd.XXXXXX) || return 1
...
iclaw_configure_node_identity() {
  local runtime_user=node
  local mode="${iclaw_HOST_IDENTITY_MODE:-unknown}"
  local requested_uid="${iclaw_HOST_UID:-}"
  local requested_gid="${iclaw_HOST_GID:-}"
  ...
  case "$mode" in
    direct)
      if ! iclaw_valid_nonroot_id "$requested_uid"; then
        iclaw_permission_fatal \
      ...
      if [ -n "$requested_gid" ]; then
        if ! iclaw_valid_nonroot_id "$requested_gid"; then
          iclaw_permission_fatal \
      ...
      if [ "$requested_uid" != "$current_uid" ] || [ "$target_gid" != "$current_gid" ]; then
        if ! iclaw_write_runtime_ids "$runtime_user" "$requested_uid" "$target_gid"; then
          iclaw_permission_fatal "could not update the node passwd identity"
          return 1
        fi
      fi
      iclaw_INTERNAL_IDENTITY_MODE=direct
      iclaw_INTERNAL_RUNTIME_UID="$requested_uid"
      iclaw_INTERNAL_RUNTIME_GID="$target_gid"
    ...
    rootless)
      ...
      iclaw_INTERNAL_IDENTITY_MODE=rootless
      iclaw_INTERNAL_RUNTIME_UID="$current_uid"
      iclaw_INTERNAL_RUNTIME_GID="$current_gid"
      ;;
    host-root)
      ...
      iclaw_INTERNAL_IDENTITY_MODE=host-root
      iclaw_INTERNAL_RUNTIME_UID="$current_uid"
      iclaw_INTERNAL_RUNTIME_GID="$current_gid"
      ;;
    virtualized)
      ...
      iclaw_INTERNAL_IDENTITY_MODE=virtualized
      iclaw_INTERNAL_RUNTIME_UID="$current_uid"
      iclaw_INTERNAL_RUNTIME_GID="$current_gid"
      ;;
    userns)
  ...
  export iclaw_INTERNAL_IDENTITY_MODE
  export iclaw_INTERNAL_RUNTIME_UID iclaw_INTERNAL_RUNTIME_GID
}

iclaw_prepare_mounted_paths() {
  case "$iclaw_INTERNAL_IDENTITY_MODE" in
    direct)
    ...
    host-root | virtualized)
      ...
      ;;
    *)
      iclaw_permission_fatal "mount preparation attempted before safe identity setup"
      return 1
      ;;
  esac
}

iclaw_prepare_generated_path() {
  local generated_key="$1"
  case "$generated_key" in
    npm-global | skills | dist | chromium) ;;
    *)
      iclaw_permission_fatal "unknown generated permission root"
      return 1
      ;;
  esac
  case "$iclaw_INTERNAL_IDENTITY_MODE" in
...
iclaw_source_runtime_env() {
  ...
  local iclaw_HOST_IDENTITY_MODE="$iclaw_HOST_IDENTITY_MODE"
  local iclaw_HOST_UID="${iclaw_HOST_UID:-}"
  local iclaw_HOST_GID="${iclaw_HOST_GID:-}"
  local iclaw_RECONCILE_SESSION_PERMISSIONS=
  local iclaw_MOUNT_PREPARE_MODE=
  local iclaw_RUNTIME_USER=
  local iclaw_SESSION_ROOT=
  local iclaw_SESSION_PERMISSION_HELPER=
  local iclaw_SESSION_PERMISSION_INTERVAL=
  local iclaw_SESSION_PERMISSION_PID=
  local iclaw_INTERNAL_IDENTITY_MODE="$iclaw_INTERNAL_IDENTITY_MODE"
  local iclaw_INTERNAL_RUNTIME_UID="$iclaw_INTERNAL_RUNTIME_UID"
  local iclaw_INTERNAL_RUNTIME_GID="$iclaw_INTERNAL_RUNTIME_GID"
  local iclaw_INTERNAL_WATCHER_PID="$iclaw_INTERNAL_WATCHER_PID"
...
iclaw_migrate_direct_managed_paths() {
  [ "$iclaw_INTERNAL_IDENTITY_MODE" = direct ] || return 0
...
iclaw_verify_rootless_bridge() {
  local mounted_path current_uid legacy_uid
  [ "$iclaw_INTERNAL_IDENTITY_MODE" = rootless ] || return 0
  legacy_uid="$iclaw_INTERNAL_RUNTIME_UID"
  for mounted_path in \
...
iclaw_start_session_permission_watcher() {
  ...
  iclaw_verify_rootless_bridge || return 1
  rm -f -- /run/iclaw-session-watcher.ready /run/iclaw-session-watcher.failed
  /usr/local/bin/node /app/session-permissions-watcher.mjs $watcher_mode &
  iclaw_INTERNAL_WATCHER_PID=$!
  for ((attempt = 0; attempt < 500; attempt++)); do
    if [ -e /run/iclaw-session-watcher.ready ]; then
      export iclaw_INTERNAL_WATCHER_PID
      return 0
    fi
    if [ -e /run/iclaw-session-watcher.failed ] || \
      ! kill -0 "$iclaw_INTERNAL_WATCHER_PID" 2>/dev/null; then
      wait "$iclaw_INTERNAL_WATCHER_PID" 2>/dev/null || true
      iclaw_INTERNAL_WATCHER_PID=
      iclaw_permission_fatal "rootless permission watcher failed before readiness"
      return 1
    fi
    sleep 0.02
  done
  kill "$iclaw_INTERNAL_WATCHER_PID" 2>/dev/null || true
  wait "$iclaw_INTERNAL_WATCHER_PID" 2>/dev/null || true
  iclaw_INTERNAL_WATCHER_PID=
  iclaw_permission_fatal "rootless permission watcher readiness timed out"
}

iclaw_stop_session_permission_watcher() {
  local watcher_status=0
  if [ -n "$iclaw_INTERNAL_WATCHER_PID" ]; then
    kill "$iclaw_INTERNAL_WATCHER_PID" 2>/dev/null || true
    wait "$iclaw_INTERNAL_WATCHER_PID" 2>/dev/null || watcher_status=$?
    iclaw_INTERNAL_WATCHER_PID=
  fi
  if [ "$watcher_status" -ne 0 ]; then
    iclaw_permission_fatal "permission watcher final scan failed"
    return 1
  fi
  case "$iclaw_INTERNAL_IDENTITY_MODE" in
    direct)

...
iclaw_prepare_generated_path npm-global
...
iclaw_prepare_generated_path skills
...
iclaw_prepare_generated_path dist
...
cleanup() {
  ...
  iclaw_stop_session_permission_watcher || cleanup_status=$?
...
if [ "$iclaw_INTERNAL_IDENTITY_MODE" = rootless ]; then
  umask 0007
fi
...
iclaw_CHROMIUM_CDP_HOST="${iclaw_CHROMIUM_CDP_HOST:-127.0.0.1}"
iclaw_CHROMIUM_CDP_PORT="${iclaw_CHROMIUM_CDP_PORT:-9222}"
CHROMIUM_PROFILE_DIR=/tmp/iclaw-chromium-profile
CHROMIUM_LOG=/tmp/iclaw-chromium.log
mkdir -p "$CHROMIUM_PROFILE_DIR"
iclaw_prepare_generated_path chromium
...
export AGENT_BROWSER_CDP="$iclaw_CHROMIUM_CDP_PORT"
...
HOME=/home/node setpriv --reuid=node --regid=node --init-groups -- \
  ...
  --remote-debugging-address="$iclaw_CHROMIUM_CDP_HOST" \
  --remote-debugging-port="$iclaw_CHROMIUM_CDP_PORT" \
...
for ((attempt = 0; attempt < 100; attempt++)); do
  if curl --noproxy '*' -fsS \
    "http://127.0.0.1:${iclaw_CHROMIUM_CDP_PORT}/json/version" \
    >/dev/null 2>&1; then
    CHROMIUM_READY=true
    break
  fi
...
if [ "$CHROMIUM_READY" != true ]; then
  echo "Chromium failed to listen on container-local CDP port ${iclaw_CHROMIUM_CDP_PORT}" >&2

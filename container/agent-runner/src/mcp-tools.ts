/**
 * MCP Tool Definitions for iclaw Agent Runner.
 *
...
export interface McpContext {
  ...
  isHome: boolean;
  isAdminHome: boolean;
  /** Whether this runtime is an interactive session of the main iclaw. */
  agentBuilderEnabled: boolean;
  /** Host admitted Owner Profile capability for built-in iclaw in Home. */
  ownerProfileEnabled: boolean;
  /** Host-authoritative HR Agent capability switch. Absent means enabled. */
  hrAgentEnabled?: boolean;
...
export interface iclawOwnerProfileProjection {
  workspaceJid: string;
...
export interface iclawOwnerProfileTurnResult {
  projection: iclawOwnerProfileProjection;
  onboardingStatus:
    | 'awaiting'
    | 'known'
    | 'cleared'
...
const OWNER_PROFILE_RESULT_PREFIX = 'iclaw_owner_profile_result';
...
async function calliclawOwnerProfile(
  ctx: McpContext,
 ...
 */
export async function acknowledgeiclawOwnerProfileFirstWake(
  ctx: McpContext,
  leaseToken: number,
  inputTurnId: string,
  ...
  try {
    const result = await calliclawOwnerProfile(
      ctx,
      'ack_first_wake',
      { leaseToken },
      timeoutMs,
...
export async function fetchiclawOwnerProfileTurn(
  ctx: McpContext,
  timeoutMs: number = 5_000,
  inputTurnId: string | null | undefined = ctx.currentInputTurnId,
): Promise<iclawOwnerProfileTurnResult | null> {
  ...
  try {
    ...
    const projection =
      result.projection as unknown as iclawOwnerProfileProjection;
    ...
    return {
      projection,
      onboardingStatus:
        onboardingStatus as iclawOwnerProfileTurnResult['onboardingStatus'],
      firstWake:
...
/**
 * Create all iclaw MCP tool definitions for in-process SDK MCP server.
 */
export function createMcpTools(ctx: McpContext): McpToolDefinition<any>[] {
  ...
  const callHrCapability = async (
    ...
    if (!result.success) {
      throw new Error(
        typeof result.error === 'string'
          ? result.error
          : 'HR capability request failed',
      );
    }
    return result;
  };

  const hrToolResult = (result: Record<string, unknown>) => ({
    content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }],
  });
  ...
  const tools: McpToolDefinition<any>[] = [
    ...
    // --- HR Agent capability broker ---
    tool(
      'hr_list_jobs',
      'List current workspace HR jobs without candidate PII. Returns active, paused, closed, expired and soft-deleted lifecycle information; deleted jobs are normally absent. Call this before matching a job name to an ID.',
      {
        status: z
        ...
        search: z
          ...
          .max(160)
          .optional()
          .describe('Title, department or location search text'),
        limit: z.number().int().min(1).max(100).optional().default(50),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('list_jobs', {
            status: args.status,
            search: args.search,
            limit: args.limit,
          }),
    ...
    tool(
      'hr_get_job_stats',
      'Get candidate funnel, stage counts and AI status counts for one HR job. Resolve job_id with hr_list_jobs first.',
      {
        job_id: z.string().min(1).max(128),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('get_job_stats', { job_id: args.job_id }),
        ),
    ),
    tool(
      'hr_get_job',
      'Read one HR job’s full JD, structured requirements, lifecycle, candidate funnel and recent job-rule summaries. Resolve job_id with hr_list_jobs first.',
      {
        job_id: z.string().min(1).max(128),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('get_job', { job_id: args.job_id }),
        ),
    ),
    tool(
      'hr_analyze_job',
      'Run a read-only quality analysis of one JD and return strengths, ambiguities, missing requirements, risks and improvements. It does not modify the job.',
      {
        job_id: z.string().min(1).max(128),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('analyze_job', { job_id: args.job_id }),
        ),
    ),
    tool(
      'hr_analyze_resume_preview',
      'Analyze a resume attached to the current chat against a temporary JD without creating HR records. Use the attachment_id from the resume attachment hint and the JD text supplied by the user. The result is a preview only; never save it as a candidate or job.',
      {
        attachment_id: z
          .string()
          .regex(/^[0-9a-f-]{36}\.(pdf|doc|docx|txt|md)$/)
          .describe(
            'Use the attachment_id from the current chat resume attachment',
          ),
        jd_text: z
          .string()
          .min(20)
          .max(60000)
          .describe('The temporary JD text to compare with the resume'),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('analyze_resume_preview', {
    ...
    tool(
      'hr_list_candidates',
      'List HR candidate business summaries for the current workspace. Results contain no phone numbers, emails or resume text. Use search or filters before changing a candidate.',
      {
        job_id: z.string().max(128).optional(),
        stage: z
          .enum([
            'pending_tech_screen',
            'interview_1',
      ...
      async (args) =>
        hrToolResult(
          await callHrCapability('list_candidates', {
            job_id: args.job_id,
            stage: args.stage,
            talent_pool: args.talent_pool,
            search: args.search,
            limit: args.limit,
          }),
        ),
    ...
    tool(
      'hr_get_candidate',
      'Get one candidate’s interview progress, score, recommendation, match evidence and recent rounds. Prefer candidate_id; full_name resolves only when unique. Never exposes PII or resume text.',
      {
        candidate_id: z.string().max(128).optional(),
        current_full_name: z.string().min(1).max(160).optional(),
        job_id: z.string().max(128).optional(),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('get_candidate', {
            candidate_id: args.candidate_id,
            full_name: args.current_full_name,
            job_id: args.job_id,
          }),
        ),
    ),
    tool(
      'hr_search_aggregate',
      'Search current workspace jobs and candidate business summaries in one call. Results exclude candidate PII, profile objects and resume text; candidate summaries are additionally masked for emails and phone numbers.',
      {
        query: z.string().trim().min(1).max(160),
        scope: z.enum(['all', 'jobs', 'candidates']).optional().default('all'),
        job_id: z.string().max(128).optional(),
        stage: z
          .enum([
            'pending_tech_screen',
      ...
      async (args) =>
        hrToolResult(
          await callHrCapability('search_aggregate', {
            query: args.query,
            scope: args.scope,
            job_id: args.job_id,
            stage: args.stage,
    ...
    tool(
      'hr_start_candidate_analysis',
      'Start the existing persistent candidate analysis queue for one candidate. Prefer candidate_id; full_name resolves only when unique. Track progress with analysis_job_id using hr_get_analysis_job.',
      {
        candidate_id: z.string().max(128).optional(),
      ...
      async (args) =>
        hrToolResult(
          await callHrCapability('start_candidate_analysis', {
            candidate_id: args.candidate_id,
    ...
    tool(
      'hr_start_job_analysis',
      'Start a market/funnel analysis for one HR job. Resolve job_id with hr_list_jobs first and track progress with hr_get_analysis_job.',
      {
        job_id: z.string().min(1).max(128),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('start_job_analysis', { job_id: args.job_id }),
        ),
    ),
    tool(
      'hr_get_analysis_job',
      'Track a candidate analysis or market analysis job. Does not expose resume text or candidate PII.',
      {
        analysis_job_id: z.string().min(1).max(128),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('get_analysis_job', {
    ...
    tool(
      'hr_update_candidate',
      'Update a candidate’s name and/or HR stage after the user explicitly confirms the target record and exact changes. If full_name resolves to multiple candidates, return the list and ask for confirmation instead of guessing. Feishu sync follows the normal configured policy.',
      {
        candidate_id: z.string().max(128).optional(),
        full_name: z.string().min(1).max(160).optional(),
        job_id: z.string().max(128).optional(),
        new_full_name: z.string().min(1).max(160).optional(),
        talent_pool: z.boolean().optional(),
        summary: z
          .string()
          .trim()
          ...
          .nullable()
          .optional()
          .describe('HR business summary; pass null to clear it'),
        stage: z
          .enum([
            'pending_tech_screen',
            'interview_1',
        ...
        expected_updated_at: z
          ...
          .min(1)
          .max(64)
          .describe('Use candidate.updatedAt from hr_get_candidate'),
        confirmed: z
          .boolean()
          .optional()
          .default(false)
      ...
      async (args) =>
        hrToolResult(
          await callHrCapability('update_candidate', {
            candidate_id: args.candidate_id,
            job_id: args.job_id,
    ...
    tool(
      'hr_list_interview_rounds',
      'List a candidate’s interview rounds and feedback previews. Prefer candidate_id; full_name resolves only when unique. Results do not include candidate PII or full feedback text.',
      {
        candidate_id: z.string().max(128).optional(),
        full_name: z.string().min(1).max(160).optional(),
        job_id: z.string().max(128).optional(),
      ...
      async (args) =>
        hrToolResult(
          await callHrCapability('list_interview_rounds', {
            candidate_id: args.candidate_id,
            full_name: args.full_name,
            job_id: args.job_id,
          }),
    ...
    tool(
      'hr_submit_interview_feedback',
      'Record one interview round and trigger the existing feedback analysis / JD rule draft flow. Show the candidate, stage and feedback summary, then set confirmed=true only after explicit approval.',
      {
        candidate_id: z.string().max(128).optional(),
        full_name: z.string().min(1).max(160).optional(),
        job_id: z.string().max(128).optional(),
      ...
      async (args) =>
        hrToolResult(
          await callHrCapability('submit_interview_feedback', {
            candidate_id: args.candidate_id,
            full_name: args.full_name,
            job_id: args.job_id,
            stage: args.stage,
    ...
    tool(
      ...
      'Manually sync one candidate’s current payload to the configured Feishu Bitable. Use the same candidate_id confirmed by hr_get_candidate. The tool preserves provider failures as failures.',
      {
        candidate_id: z.string().min(1).max(128),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('sync_candidate_to_feishu', {
            candidate_id: args.candidate_id,
          }),
        ),
    ),
    tool(
      'hr_update_job',
      'Update one HR job’s title, JD text or structured requirements after the user explicitly confirms the proposed diff. Resolve job_id, call hr_get_job, show the proposed changes, then set confirmed=true only after approval. This tool cannot change status, expiry, deletion or restore.',
      {
        job_id: z.string().min(1).max(128),
        expected_updated_at: z
          .string()
          ...
          .max(64)
          .describe(
            'Use job.updatedAt from hr_get_job to prevent concurrent overwrite',
          ),
        ...
        jd_text: z.string().min(1).max(60000).optional(),
        keywords: z.array(z.string().trim().min(1).max(80)).max(100).optional(),
        responsibilities: z
        ...
        confirmed: z
          ...
          .default(false)
          .describe(
            'Must be true only after the user explicitly approves the diff',
          ),
      },
      async (args) =>
        hrToolResult(
          await callHrCapability('update_job', {
            job_id: args.job_id,
            expected_updated_at: args.expected_updated_at,
            title: args.title,
...
Returns null if the current chat is a DM (DMs do not belong to a server). Only works when the current chat is a Discord channel.`,
  ...
  if (ctx.agentBuilderEnabled) {
    ...
    tools.push(
      tool(
        'agent_profile_list',
        "List the current user's top-level Agents and resumable ready drafts. Use this before editing or resuming work so you can identify the target, current version, draft ID, and draft revision. The main iclaw is returned for context but cannot edit itself with Agent Builder.",
        {},
        async () => {
...
Use the skills panel in the UI to find the skill ID (directory name, e.g. "memory", "think").`,
  ...
  const genericMemoryCanonicalKeySchema = z
    ...
    .max(300)
    .refine(
      (value) => value.trim() !== 'iclaw.owner.preferred_address',
      'Use iclaw_owner_profile for the reserved owner address field',
    );
  ...
  // HR capability remains host-gated; older hosts without the flag keep the
  // previous enabled-by-default behavior.
  if (ctx.hrAgentEnabled === false) {
    return tools.filter((tool) => !tool.name.startsWith('hr_'));
  }
  return tools;
}

/**
 * iclaw's production Agent Runner entry point.
 *
...
import { buildiclawPromptPlan } from './prompt-plan.js';
...
const OUTPUT_START = '---iclaw_OUTPUT_START---';
const OUTPUT_END = '---iclaw_OUTPUT_END---';
...
function buildSystemPrompt(input: ContainerInput, ctx: McpContext): string {
  ...
  const plan = buildiclawPromptPlan({
    platformIdentity: input.agentProfile?.isDefault
      ? loadPrompt('identity.iclaw.md')
      : undefined,
    platformBootstrap:
      input.agentProfile?.isDefault && input.iclawOwnerProfileEnabled
        ? loadPrompt('bootstrap.iclaw.md')
        : undefined,
...
async function runTurn(
  ...
  const [memory, owner] = await Promise.all([
    ...
    loadiclawOwnerProfileTurnContext(() =>
      fetchiclawOwnerProfileTurn(ctx),
    ),
  ]);
  if (!memory.snapshot)
    logWarn(
    ...
    emit: (output) => {
      if (
        ...
        if (
          ...
          void acknowledgeiclawOwnerProfileFirstWake(
            ctx,
            leaseToken,
...
async function main(): Promise<void> {
  ...
  const ctx: McpContext = {
    ...
    ownerProfileEnabled: input.iclawOwnerProfileEnabled === true,

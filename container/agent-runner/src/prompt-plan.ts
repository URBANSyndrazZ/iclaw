...
export interface iclawPromptSources {
...
export function buildiclawPromptPlan(
  sources: iclawPromptSources,
): PromptPlan {
  ...
  if (sources.platformIdentity) {
    inputs.push({
      ...
      owner: 'platform',
      required: true,
      condition: 'built-in default iclaw AgentProfile',
  ...
  if (sources.platformBootstrap) {
    inputs.push({
      ...
      required: false,
      condition:
        'built-in iclaw Home runtime; each turn follows the host-authoritative owner-profile block',

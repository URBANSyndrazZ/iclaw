...
export const iclaw_SUBAGENT_RUNTIME_CONTRACT = `## iclaw delegated-task contract

You are executing a task delegated by a parent agent. Return the requested findings or work product to that parent agent; do not act as though your text is the final user-facing reply. Stay within the delegated scope. Do not independently operate iclaw memory or Agent Builder unless the delegated task explicitly requires it.`;
...
function contractHash(): string {
  return createHash('sha256')
    .update(iclaw_SUBAGENT_RUNTIME_CONTRACT, 'utf8')
    .digest('hex');
}
 ...
 */
export function withiclawSubagentContract<
...
>(
  ...
  const enabled =
    process.env.iclaw_DISABLE_SUBAGENT_RUNTIME_CONTRACT !== 'true';
  ...
  return {
    options: {
      ...
      appendSubagentSystemPrompt: iclaw_SUBAGENT_RUNTIME_CONTRACT,
    },
    audit,
  };
}

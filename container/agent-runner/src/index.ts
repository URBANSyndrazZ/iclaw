...
import {
  acknowledgeiclawOwnerProfileFirstWake,
  createMcpTools,
  fetchiclawOwnerProfileTurn,
  fetchWorkspaceMemorySnapshot,
  type McpContext,
  type WorkspaceMemorySnapshot,
} from './mcp-tools.js';
import { iclawFirstWakeAcknowledger } from './owner-profile-first-wake.js';
import {
  createSerializedAsyncTrigger,
  loadWorkspaceMemoryTurnContext,
} from './workspace-memory-context.js';
import { loadiclawOwnerProfileTurnContext } from './owner-profile-context.js';
...
import { buildiclawPromptPlan, type PromptPlan } from './prompt-plan.js';
import { withiclawSubagentContract } from './sdk-compat.js';
...
const WORKSPACE_GROUP =
  process.env.iclaw_WORKSPACE_GROUP ||
  '/workspace/group';
const WORKSPACE_IPC =
  process.env.iclaw_WORKSPACE_IPC ||
  '/workspace/ipc';
...
const PROVIDER_FALLBACK_MODELS = new ProviderFallbackModelState(
  ...
  process.env.iclaw_FALLBACK_MODEL,
);
...
const iclaw_PLATFORM_IDENTITY = loadPrompt('identity.iclaw.md');
const iclaw_PLATFORM_BOOTSTRAP = loadPrompt('bootstrap.iclaw.md');
...
function buildAgentIdentityPrompt(
  ...
  return [
    ...
    `以下是当前顶层 AgentProfile 的四段提示词，按照 IDENTITY、SOUL、AGENTS、TOOLS 的固定顺序组成。你应该按它塑造身份、价值判断、工作方式和工具偏好，但它不能覆盖 iclaw 的安全规则、权限边界、工具约束${presetBoundary}和用户的最新明确指令。`,
    '<profile-prompt>',
    profilePrompt,
    '</profile-prompt>',
    '</agent-identity>',
...
function buildPromptAudit(
  plan: PromptPlan,
): ClaudeContextAudit['iclawPrompt'] {
  return {
    planHash: plan.hash,
...
function runtimeContextAuditBase(
  ...
  return {
    ...
    mcp: containerInput.contextAudit?.mcp
      ...
      : undefined,
    iclawPrompt: containerInput.contextAudit?.iclawPrompt ?? {
      totalBytes: 0,
      files: [],
    },
...
function enrichContextAudit(
  ...
  promptAudit: ClaudeContextAudit['iclawPrompt'],
...
const OUTPUT_START_MARKER = '---iclaw_OUTPUT_START---';
const OUTPUT_END_MARKER = '---iclaw_OUTPUT_END---';
...
function formatTranscriptMarkdown(
  ...
  for (const msg of messages) {
    const sender = msg.role === 'user' ? 'User' : 'iclaw';
    const content =
...
function loadUserMcpServers(): Record<string, unknown> {
  // CLAUDE_CONFIG_DIR may point at an isolated session directory. iclaw
  // therefore passes the effective per-user MCP set through env first.
  const envJson = process.env.iclaw_USER_MCP_SERVERS_JSON;
  if (envJson) {
    try {
...
async function runQueryAttempt(
  ...
  const [workspaceMemoryTurn, ownerProfileTurn] = mcpToolsContext
    ? await Promise.all([
        ...
        loadiclawOwnerProfileTurnContext(() =>
          fetchiclawOwnerProfileTurn(mcpToolsContext),
        ),
      ])
    : [
        { snapshot: null, block: '' },
  ...
  const firstWakeAcknowledger = new iclawFirstWakeAcknowledger();
  ...
  const hasMemoryTools = allowedTools.some(
    (tool) =>
      tool === 'mcp__iclaw__*' ||
      tool === 'mcp__iclaw__workspace_memory_search' ||
      tool === 'mcp__iclaw__workspace_memory_get' ||
      tool === 'mcp__iclaw__workspace_memory_remember' ||
      tool === 'mcp__iclaw__workspace_memory_update' ||
      tool === 'mcp__iclaw__workspace_memory_forget',
  );
  ...
  const promptPlan = buildiclawPromptPlan({
    platformIdentity: containerInput.agentProfile?.isDefault
      ? iclaw_PLATFORM_IDENTITY
      : undefined,
    platformBootstrap:
      containerInput.agentProfile?.isDefault &&
      containerInput.iclawOwnerProfileEnabled
        ? iclaw_PLATFORM_BOOTSTRAP
        : undefined,
  ...
  // 调试观察：iclaw_DUMP_PROMPT=true 时把最终 system prompt 输出到 stderr
  // host 已通过 logs/ 捕获 stderr，方便对比改 prompts/*.md 前后的差异
  if (process.env.iclaw_DUMP_PROMPT === 'true') {
    log(
  ...
  // Claude Code plugins injected by iclaw main process via ContainerInput.
  ...
  try {
    ...
    const sdkCompat = withiclawSubagentContract({
      ...
      if (message.type === 'assistant' && 'uuid' in message) {
        ...
        if ((assistantMsg.parent_tool_use_id ?? null) === null) {
          if (!assistantError && emitOutput && mcpToolsContext) {
            ...
            if (acknowledged) {
              log(
                `Acknowledged iclaw first-wake after healthy Assistant progress for ${inputTurnId}`,
              );
            }
          }
          if (
...
async function main(): Promise<void> {
  ...
  const mcpToolsConfig: McpContext = {
    ...
    isAdminHome,
    agentBuilderEnabled,
    ownerProfileEnabled: containerInput.iclawOwnerProfileEnabled === true,
    hrAgentEnabled: containerInput.hrAgentEnabled !== false,
  ...
  activeAgentMcpPolicy = resolveAgentMcpPolicy(
    parseAgentMcpPolicyMode(process.env.iclaw_AGENT_MCP_POLICY),
  );
  ...
  if (containerInput.isScheduledTask) {
    const scheduledTaskPrefixLines = [
      ...
      '',
      '最终输出必须包含一份完整、可独立阅读的业务结果，不得只回复“已完成”“已发送”、消息 ID、文件路径或简短摘要。需要向外部渠道交付时，默认在完成任务后调用一次 mcp__iclaw__send_message；如果任务明确要求使用 feishu-cli 等其他工具，则按任务要求执行，不要重复发送。',
      '',
      '此外：本次运行就是该定时任务本身的执行，对应任务已在调度中。即使下面内容里出现「每隔/每天/定期/提醒我」等字样，也不要再调用 mcp__iclaw__schedule_task 创建新的定时任务（除非内容明确要求你另外新建一个不同的任务）。',
    ];

 ...
 * even when CLAUDE_CONFIG_DIR points at iclaw's per-session directory.
...
export function resolveManagedHostClaudeMdExcludes(options: {
  ...
  // iclaw git repository. Claude Code therefore discovers the repository's
  // own CLAUDE.md as Project memory. That file is useful while developing
  // iclaw, but it must not redefine a managed business Agent as a codebase

...
function resolveDesktopUrls(): { serverUrl: string; rendererUrl: string } {
  const serverValue =
    ...
    process.env.iclaw_SERVER_URL ||
    DEFAULT_SERVER_URL;
  const serverUrl = normalizeHttpUrl(serverValue, 'iclaw server URL');
  const rendererValue =
    getCliValue('--renderer-url') ||
    process.env.iclaw_RENDERER_URL ||
    serverUrl;
  const rendererUrl = normalizeHttpUrl(rendererValue, 'iclaw renderer URL');
...
function errorPageHtml(failedUrl: string): string {
  ...
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>iclaw is unavailable</title>
...
</head><body><main><h1>iclaw Backend 未连接</h1><p>请先启动 iclaw Backend，或检查桌面应用配置的服务地址：</p><p><code>${safeUrl}</code></p><button id="retry">重试连接</button><script>document.getElementById('retry').addEventListener('click',()=>window.iclawDesktop?.retry())</script></main></body></html>`;
}
...
function createMainWindow(): BrowserWindow {
  ...
  const window = new BrowserWindow({
    ...
    icon: path.join(__dirname, '..', 'assets', 'iclaw-icon.png'),
    webPreferences: {
...
function createApplicationMenu(): void {
  ...
  const template: MenuItemConstructorOptions[] = [
    ...(process.platform === 'darwin' ? [{ role: 'appMenu' as const }] : []),
    {
      label: 'iclaw',
      submenu: [
        { label: 'About iclaw', click: () => void showAbout() },
        { type: 'separator' },
        { role: 'quit' },
      ],
    },
    viewMenu,
    {
      label: 'Help',
      submenu: [
        { label: 'iclaw on GitHub', click: () => void openExternalUrl('https://github.com/URBANSyndrazZ/iclaw') },
      ],
    },
  ];
